...
- **`rounded-full` is prohibited**, with only the following four exceptions:
  ...
  4. The dock's **collapsed input group** (the bare one-row input box — the FLORA Chat-bar stadium, user-ruled 2026-09-02). The stadium is correct geometry ONLY on the collapsed box; any second band (run-status shimmer / staged chips) morphs it back to `rounded-xl`, and the question pill stays `rounded-xl` in all forms. **输入框独立层律（同日拍板）**：the input group is always a standalone layer, never fused with the message flow — the history region floats as its OWN frosted layer above it (same dock-surface family as the question pill), so an open history no longer breaks the stadium.
...
- **Border radius**: global small radius (`rounded-md` / `rounded-lg`), avoid `rounded-full` (except for circular icon buttons, red dots, the home composer's docked one-line stadium bar, and the dock's collapsed one-row input group).
