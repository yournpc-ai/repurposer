...
- **`DialogContent` centers via `-translate-1/2`** — a transformed popup becomes the containing block for `fixed` descendants, teleporting viewport-anchored floaters (`MentionPicker`). A popup that hosts the MentionEditor composes `DialogPortal` + `DialogOverlay` + `DialogPrimitive.Popup` by hand and centers with `inset-0 m-auto` (no transform). (Precedent: `RecipeInspectOverlay`.)
...
> **ADR-038（2026-08-08 已决）**：Brand 降为人设的皮肤块（`personas.brand`，全栈一词——模块退役词不退役），`/brand-template` 并入人设页皮肤分区，composer Brand pill 退役，config 按皮肤/工艺/格式三分流——本节描述迁移前现状；第一刀（全栈改名 Speaker→Persona，08-09）已落地，皮肤吸收随第二刀（插入周 08-10 起）更新本节。实施简报 `docs/tasks/persona-identity.md`。
...
- **Brand = visual skin only**：voice / audience / contentGuidelines / CTA 归属人设（`/personas/$id`），生成时经 `GenerationContext.persona` 注入 Content Director / Clip Agent。理由：同一个人设可以服务多个 Brand（如大学官方号 vs 个人 IP），内容策略必须跟人走而不是跟皮肤走。
