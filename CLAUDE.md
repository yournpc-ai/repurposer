...
- **Hero surfaces are fully flat**: the home composer is a white flat card on the gray page — the base primitive's `ring-foreground/10` hairline only, NO shadow (the fill step does the lifting). Its entities are bottom-row pills, not straddle blocks (retired 2026-08-20, ADR-046 — see Composer section). **No `backdrop-filter` on the composer card**: it would make the card the containing block for the fixed `MentionPicker` inside the editor (teleporting it). Media tiles (recipe gallery) are fully chromeless: no ring, no shadow — the media content is its own separation. **Gallery card state machine**: rest = the poster (capability chip + aspect badge, no autoplay) → hover = the teaser plays **WITH SOUND by default** (2026-08-21 ruling — unmuted `play()` first; the browser gesture policy is the only gate: a rejected unmuted play falls back to muted and the toggle honestly reflects the effective state; any click, the toggle included, grants activation so sound lands from the next hover on) **plus the action trio** (MiniMax anatomy): the sound toggle takes the aspect badge's bottom-LEFT slot (the badge is rest-only chrome — its slot transforms on hover), a white stadium **Remix pill** (`Wand2` + label) centers on the media, and an expand button sits bottom-right — Remix and expand open the same inspect overlay, so hover adds affordance without a second launch path → click = the inspect overlay (the only launch path, ADR-040).
