...
- **`rounded-full` is prohibited**, with only the following four exceptions:
  ...
  4. The dock's **collapsed input group** (the bare one-row input box — the FLORA Chat-bar stadium, user-ruled 2026-09-02). The stadium is correct geometry ONLY on the collapsed box; any second band (run-status shimmer / staged chips) morphs it back to `rounded-xl`, and the question pill stays `rounded-xl` in all forms. **输入框独立层律（同日拍板）**：the input group is always a standalone layer, never fused with the message flow — the history region floats as its OWN frosted layer above it (same dock-surface family as the question pill), so an open history no longer breaks the stadium. **2026-09-06 起本条例外收窄为移动端 dock**（ADR-056 三形态机）：桌面 run 后 chat 进右侧 docked 面板，其输入组恒 `rounded-xl`；桌面法则 = 输入钉面板底寄存、历史 = 面板滚动区（独立层律的意图由面板解剖天然兑现）。
