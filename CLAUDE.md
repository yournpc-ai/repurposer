...
- **Hero surfaces are fully flat**: the home composer is a white flat card on the gray page — the base primitive's `ring-foreground/10` hairline only, NO shadow (the fill step does the lifting). Its entities are bottom-row pills, not straddle blocks (retired 2026-08-20, ADR-046 — see Composer section). **No `backdrop-filter` on the composer card**: it would make the card the containing block for the fixed `MentionPicker` inside the editor (teleporting it). Media/schematic tiles (recipe gallery) are fully chromeless: no ring, no shadow — the fill step does the separating (schematic covers sit in an `inset` well, lines consume `foreground` alphas). **Gallery card state machine (ADR-048, 2026-08-23)**: the card face is a **monochrome schematic process diagram, never real footage** — rest = static line-art cover (inline SVG per recipe id under `components/recipes/covers/`; three gray tiers: structure `foreground/40` / figure `foreground/85`-class / result solid foreground; token-driven so the light theme inverts for free; 16:10 tile; composition reads LEFT=input → RIGHT=output) → hover = the diagram **plays its process animation** (CSS keyframes on SVG groups — zero video, zero sound machinery, zero media requests on the home page) + the action duo: a white stadium **Remix pill** (`Wand2` + label) centered, expand button top-right — both open the same inspect overlay → click = the inspect overlay (the only launch path, ADR-040). **Evidence lives in the overlay**: the grid carries no real content at rest; real paired before/after examples are the Examples tab's job (acceptance criterion — a card without real paired examples does not enter the grid; Soon/reserved cards no longer exist). Text sits BELOW the tile in three grid-aligned rows: 菜名 / promise (2-line clamp) / 适用素材 (meta). **No aspect badge** (cards never lock output shape — shape claims live in the promise copy and the overlay's real examples), **no category chip** (the input anchor is the 适用 row), **no channel names on the face** (genre only — 社媒帖， not "LinkedIn 帖"; channel = publish/plan-time variable: the prefill template carries the default, chat overrides). Grid = uniform 4 columns (`lg`) / 3 (`md`) / 2 (mobile) — no featured spans (editorial priority rides order); MasonryGrid retired. Card admission gate (RECIPES §4.8): ① real high-frequency expert scenario (fabricated scenarios vetoed) ② non-replaceable output form, or loop value stated on the face (the ChatGPT test). The 2026-08-21 hover-with-sound ruling relocated to the overlay's Examples tab (click = gesture, sound is native there) — its grid surface no longer exists.
...
- Structure (ADR-046; straddle blocks retired 2026-08-20; **shell = the shadcn `InputGroup` since 2026-08-21** — the layout converged to the canonical AI-composer anatomy, so the component is adopted with the card-law re-skin: stock `border-input` stroke / `bg-input/20` fill give way to `border-0` + `bg-card` + `ring-foreground/10` hairline + NO shadow; what we keep = the block-start/block-end addon anatomy, the focus-within ring machinery (the `MentionEditor` root carries `data-slot="input-group-control"`), and cursor-text click-to-focus. **Density lives ON the addons, never the container**: chips = block-start `px-5`, editor band = `px-5 py-5`→`py-4` (morphing), control row = block-end `px-5` — 20px sides, 12px chin, both addons fold by interpolating maxHeight AND their one-sided padding in the same style object (a flex item's padding SURVIVES `max-height: 0` — border-box does not clip it, 2026-08-21 headless probe — so static padding classes on a fold addon are banned): the card is three bands — **staged asset chips at the top** (typed anatomy: video = thumbnail sliver + duration, audio = waveform icon + duration, doc/slides = file icon + format label — page counts would take pdf.js, never worth a chip; uploading = spinner + percent; × removes and deletes the asset), **the `MentionEditor` input band**, and **the bottom control row**. **Home parks the composer center-stage at rest and docks it as the one-line explore bar on scroll** (ADR-046 walkthrough pass 2, MiniMax parity): at rest the composer sits in the middle of the viewport under the hero (the `pt-[20vh]` stage offset); scrolling slides it up until it sticks at the top (sticky chrome inside the single scrollport — a `::before` page-fill + dot-grid backdrop fades in on dock so wider cards never peek beside it, 24px bottom mask dissolves passers) and morphs **compact** via its `compact` prop — the one-line **stadium** (half-height radius, the third user-ruled `rounded-full` exception): attach button (folded to zero width when expanded; opens `side="bottom"` — docked at the viewport top), single-line input (`h-24`→`h-14`), send. **Padding is asymmetric** (MiniMax anatomy, measured off the reference 2026-08-21): 20px sides / 20px chips top / 12px bottom chin. The chips band and the control row fold; **send is an absolute bottom-right anchor in both forms so the fold never takes it**. **The morph is SCROLL-LINKED, never a clock transition** (walkthrough pass 3 — time-based transitions lagged fast scrolls, leaving a half-melted stadium mid-screen): `dockP` = scroll progress over the last 140px before the dock point (a pure function of scrollTop — no thresholds, no hysteresis), and every morph property is an interpolation of it (padding 20/20/12→16px, editor band 96→56px, chips/control-row fold, radius 16→40px, send anchor 12/12→10px, backdrop opacity) — **the radius rides it too** (2026-08-21 reversal): 16px at rest (`rounded-2xl`, MiniMax's soft rectangle) → 40px at the dock, where the CSS radius cap (≤ half the 56px bar = 28px) turns it into the stadium; the radius is driven as an inline style, which also sidesteps the stock `has-data-[align=*]:rounded-md` pins (their `:has()` specificity outranks any plain `rounded-*` class); the title alone keeps a binary size class at full dock. **The core hero — the BRAND LOCKUP (`LogoMark` + "Repurposer" wordmark, the mark sized in em so it scales with the font) — PERSISTS, docking smaller above the bar (`text-3xl/sm:text-4xl` → `text-xl`); the category line folds away (fade + measured-height collapse, scroll-linked inside the chrome — the fold sits below the chrome's top edge so the docking point never moves). The rest offset is a `h-[28vh]` spacer sibling (padding inside the sticky chrome would pin too). 
- Entities are **bottom-row left ghost pills**, not edge-straddling blocks: `Assets` (paperclip + label + value) and `Persona` (16px avatar circle + label + value). **Pill value state law**: at rest the value sits in `meta-foreground` ("Optional" / "Auto"); once set it turns `foreground` (file count / persona name, Persona's avatar circle fills) — that single color step is the whole state change (no fills, no accent color). Pills open **frosted `Popover` panels** (`side="bottom"` — they open DOWNWARD over the gallery; the expanded composer sits mid-viewport and an upward panel covered the input itself, 2026-08-21 ruling; overlay-surface + the floater whisper shadow; scroll lists are `no-scrollbar`): the Assets panel = upload row + typed file rows + × — **row anatomy (the 2026-08-21 mock, promoted)**: a square typed tile (h-9 thumb or `bg-icon-chip`) + name / typed-meta two lines (AV = kind · duration · size, image = kind · size, doc = format · size; `formatFileSize` in `lib/stagedFiles.ts`) + a vertically-centered ×; **column law: file tiles are SQUARE, identity tiles are ROUND** (**one-place law**: the panel IS the chips band's expanded form — while it's open the band folds away, so the file list lives in exactly one place; the pill's count stays as the anchor); the Persona panel = Auto row + persona rows (same two-line anatomy: round h-9 identity tile — avatar / Auto circle chip — + name / tags · voice-binding meta) + manage link. `AssetsModal` / `PersonaPickerModal` are retired — deep asset management belongs to a future asset-center page, not the composer; persona editing still lives on `/personas`.
- Bottom row is **one continuous row inside the card** (no separate action-bar strip / muted background): entity pills left, the circular send button on the right, controls at `h-9`. The composer has **no language / outputs / clip-count controls and runs no inference of its own** (see behavioral contract). **No model control** (the display-only pill retired 2026-08-22): the pipeline assigns models per modality (文案/配音/图像音乐各走各的 provider), so the composer can't decide — or honestly display — the final model; a picker lands only when a real second provider exists, and its user-facing form is a **policy switch** (e.g. "prefer EU-hosted models"), not a model SKU shelf (裁定见 PROGRESS 需求池 "LLM provider 抽象").
...
- **Auth dialog is not an exception** (判词 2026-08-24): `LoginDialog` no longer suppresses the toast via `{toast: false}` or carries its own `error` state — every auth failure (send-code cooldown, wrong verify code, network) flows through the global sonner like any other API call. Form-specific UX (clearing digits, refocusing on a wrong code) stays in the handler — that's input reset, not error display.
