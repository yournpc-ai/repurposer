...
- **Hero surfaces mirror the floating-layer chrome in solid form**: the home composer card takes the DialogContent recipe — the base primitive's `ring-foreground/10` hairline + `shadow-xl` (light only; dark = hairline over the tonal step) — and its entity blocks (Assets / Persona) are solid chips with the same hairline (`bg-card` light / `bg-muted` dark, solid-fill hovers since they straddle the card edge). **No `backdrop-filter` on the composer card**: it would make the card the containing block for the fixed `MentionPicker` inside the editor (teleporting it), and the home page behind is a uniform fill with nothing to blur. Media tiles (recipe gallery) are flat too: hairline only, no shadow.
