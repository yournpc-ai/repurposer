...
- Structure (ADR-046; straddle blocks retired 2026-08-20): the card is three bands — **staged asset chips at the top** (typed anatomy: video = thumbnail sliver + duration, audio = waveform icon + duration, doc/slides = file icon + format label — page counts would take pdf.js, never worth a chip; uploading = spinner + percent; × removes and deletes the asset), **the `MentionEditor` input band**, and **the bottom control row**. **Home parks the composer center-stage at rest and docks it as the one-line explore bar on scroll** (ADR-046 walkthrough pass 2, MiniMax parity): at rest the composer sits in the middle of the viewport under the hero (the `pt-[20vh]` stage offset); scrolling slides it up until it sticks at the top (sticky chrome inside the single scrollport — a `::before` page-fill + dot-grid backdrop fades in on dock so wider cards never peek beside it, 24px bottom mask dissolves passers) and morphs **compact** via its `compact` prop — the one-line **stadium** (half-height radius, the third user-ruled `rounded-full` exception): attach button (folded to zero width when expanded; opens `side="bottom"` — docked at the viewport top), single-line input (`h-24`→`h-10`), send. **Padding is asymmetric** (MiniMax anatomy): `px-5 pt-5 pb-3` — generous top/sides, a tight bottom chin the control row sits close to. The chips band and the control row fold; **send is an absolute bottom-right anchor in both forms so the fold never takes it**. **The morph is SCROLL-LINKED, never a clock transition** (walkthrough pass 3 — time-based transitions lagged fast scrolls, leaving a half-melted stadium mid-screen): `dockP` = scroll progress over the last 140px before the dock point (a pure function of scrollTop — no thresholds, no hysteresis), and every morph property is an interpolation of it (padding 20/20/12→8px, editor 96→40px, chips/control-row fold, send anchor 20/12→10px, backdrop opacity) — **the radius is NOT one of them**: the card carries a CONSTANT `40px` radius (big and soft at rest, MiniMax anatomy), and the docked bar (8×2+40 = 56px tall) lets the CSS radius cap scale it to half the bar height — the stadium emerges from the box model with zero interpolation; the title alone keeps a binary size class at full dock. **The core hero — the BRAND LOCKUP (`LogoMark` + "Repurposer" wordmark, the mark sized in em so it scales with the font) — PERSISTS, docking smaller above the bar (`text-3xl/sm:text-4xl` → `text-xl`); the category line folds away (fade + measured-height collapse, scroll-linked inside the chrome — the fold sits below the chrome's top edge so the docking point never moves). The rest offset is a `h-[28vh]` spacer sibling (padding inside the sticky chrome would pin too). 
