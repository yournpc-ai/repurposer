...
- `docs/DIALOG_WORKFLOW.md` — **对话→生产概念架构母文档（ADR-052，B1~B4 已收口 2026-09-04）**：厚 agent = 产品层一个 agent、身体 = 一条 workflow（聊天 → 意图路由 → brief/计划 → 生产 DAG → 产出）；双引擎分离 / canonical 词汇（router·understand·plan·brief）/ 有界 loop 节点三护栏 / 施工切分 B1~B4。动 chat 意图层 / plan path / 多轮对话 / agent 概念前必读。
...
- **`rounded-full` is prohibited**, with only the following four exceptions:
  ...
  4. ~~The dock's collapsed input group~~ **RETIRED 2026-09-06**（随两行流式解剖落地——dock 输入组 rest 即「满宽文本带 + 流入控制条」两带，stadium 只属于真单行盒；输入组全形态恒 `rounded-xl`，法规本身对真单行盒仍然有效）。**输入框独立层律（2026-09-02 拍板，照旧）**：the input group is always a standalone layer, never fused with the message flow — the history region floats as its OWN frosted layer above it (same dock-surface family as the question pill)。**ADR-056 三形态机桌面法则照旧**：输入钉面板底寄存、历史 = 面板滚动区（独立层律的意图由面板解剖天然兑现）。
  5. The home gallery's **filter pill row**（stadium pills，user-ruled 2026-09-24，MiniMax parity——active = `bg-foreground` 深底 pill，inactive = `bg-card` 白 chip 靠 fill 步浮在灰页上）。筛选行是 sticky chrome 的第三寄存器（骑 chrome 吸顶，零自养动画），按素材维度过滤（`GALLERY_FILTERS`，宽槽卡多归属）。
...
- Controls in the same row must align on the row's center line: action-area controls are uniformly `h-9`, matching the send button height. ~~Exception (2026-08-31)~~ **RETIRED 2026-09-23（用户拍板）**：composer 底排整排统一 `h-9`（圆钮 / cost-confirm pill / send 同高，glyph 统一 `size-4.5`）——hover 背景上下对齐由同高保证，「32px 低一个 register、尺寸差即层级」旧律废止。底排叶子组件已统一收进 `components/composer/`（`ComposerIconButton` / `ComposerPanelButton` / `ComposerSendButton` / `CostConfirmControl`），home 壳与 chat dock 壳共用；glyph 左缘对齐律随 36px 钮重算为 `-ml-[9px]`（dock 的 12px rail 对应 `ml-[3px]`/`mr-[3px]`，同一法律两个取值）。
...
- **Text is three tiers, not two**: `foreground` (0.145 / dark 0.95) → `muted-foreground` subtitles (0.556 / dark 0.63) → **`meta-foreground`** (0.68 / dark 0.42 — FLORA-measured 2026-09-06: their dock disclaimer #606060 ≈ 0.42, the whole dark whisper tier moved with it, was 0.52) for small ALL-CAPS tracked meta labels (the "MODEL / SEED" style) and whisper sentences (the dock honesty line), composed via the `text-meta` utility (`uppercase` + `tracking-[0.08em]` + the token color).

### Floating Layers: frosted glass via `overlay-surface`
- **All floating layers** (Popover / DropdownMenu / Dialog / Select / Sheet / Tour) use the shared `overlay-surface` utility defined in `styles.css` — translucent `--popover` + `backdrop-blur` + `saturate(1.4)`, **per-theme recipe**: light = **92% + blur(24px)** (lower opacity lets dark content behind bleed through and tint the panel); dark = **68% + blur(28px)** (over the near-black canvas a high-opacity fill reads as a solid box and the blur has nothing to show — FLORA's 90%/12px was A/B-tried 2026-09-06 and closed same-day: on our calm near-black canvas the composite differs by ~2 lightness points of neutral gray, imperceptible; their recipe reads because their canvas is busy, ours is deliberately calm). Solid fallback under `@supports not (backdrop-filter)`. **Shadow law (ADR-046)**: the utility carries the whisper `shadow-xl` in light (lifts the glass off the content beneath) and no shadow in dark (the existing `--tw-shadow-color` switch). Do **not** re-add `bg-popover` to overlay components, and **per-instance frost patches (e.g. `dark:bg-white/10`) are banned** — tune the shared recipe; if a specific instance genuinely needs different opacity, override via its `className` (components merge it last, keeping it the open extension point).
...
- **Entity buttons**（底排左，2026-08-30 从 pill 改制，Lovart 式）：`Assets`（**Plus**——2026-09-06 自回形针换，ElevenLabs/FLORA 的 + 语义）/ ~~`Persona`（人像）~~（**hidden 2026-09-23**，`SHOW_PERSONA` 旗标留座，随定位/memory 运营端迭代回归）= **纯圆形图标按钮**（`variant="ghost"` **`h-9 w-9` 36px + `size-4.5` 18px glyph**——2026-09-23 同高裁定：整排与 send 同高，原 32px「低一个 register」律废止——`rounded-full` + 功能 Tooltip；实现 = 共享叶子 `components/composer/ComposerPanelButton`），**完全无状态**——无计数、无 Auto/人名 value 文本，选中态只在各自弹窗面板里读（钉顶条同样无文件计数）；暂存文件的可见性 = 既有 chips 带（行为不变）。**glyph 左缘对齐律（随 36px 钮重算，2026-09-23）**：左钮组带 `-ml-[9px]`——18px glyph 居中于 36px 钮内（左偏 9px），负边距让首个 glyph 左缘与编辑器文字左缘像素级对齐（hover 圆底向左溢出属正常；测量锚 = `data-tour="composer-assets"` 的扩态钮，钉顶条隐藏 attach 钮会污染测量）。**按钮开 frosted `Popover` `side="bottom"`**（向下开，扩态卡 mid-viewport 上开盖输入区；overlay-surface + 浮层耳语 shadow；scroll 列表 `no-scrollbar`）。**Assets 面板** = 上传行 + 文件行 + ×；行解剖 = h-9 类型 tile（方 = 文件 / 圆 = 身份）+ 名/类型化 meta 双行（AV = kind · duration · size，`formatFileSize` ∈ `lib/stagedFiles.ts`）+ 垂直居中 ×；**一置律**：面板 = chips 带展开态，开则带收，文件列表只在面板。**Persona 面板** = Auto 行 + 人设行（avatar / Auto 圈 chip 圆 tile + 名 / tags · 声音绑定 meta）+ manage 链。面板组件住 `components/composer/`（AssetsPanel / PersonaPanel / ModelsPanel，2026-09-23 自 components/home/ 迁入）。`AssetsModal` / `PersonaPickerModal` 退役（深资管属未来资产中心页；人设编辑仍走 `/personas`）。
...
- **Teaching 在 Tour，placeholder 只带示例**：placeholder = **固定前缀 + 3 条最常见 prompt 轮换**（Lovart 式，2026-08-30）：前缀 "Ask Repurposer to " / "让 Repurposer "（`home.placeholderPrefix`），后缀在 `home.placeholderPrompts` 三条间每 3.5s 轮换（React overlay 实现——CSS `attr()` placeholder 不能动画；仅空编辑器可见，padding 随 dockP 插值）。**过渡 = 滚动窗非闪烁**（2026-08-31 用户裁定，首版 blink-swap 退役）：旧行上滚出 + 新行下滚入，同向同时长同缓动（`styles.css` `.placeholder-roll-in/-out` 各 0.5s；`prev` 保持出场行挂载，absolute 不占布局，下行 key 重挂载复播；`prefers-reduced-motion` 出场行 display:none 退化为纯切换）；示例文案 = 直给动词 + 具体名词，保持单行长度（钉顶条只露一行）。用法教学仍归 Tour：新手 = ~~4 步~~ **3 步 Tour（2026-09-23 用户裁定：persona 入口隐藏，随定位/memory 运营端迭代回归——sidebar 的 personas 导航与 composer 的 Persona 钮同批隐藏，`SHOW_PERSONA` 旗标留座）**（assets → prompt+send → recipe gallery，`data-tour="composer-*"` / `data-tour="home-recipes"`）；**末步锚 = 第五张配方卡**（2026-08-31 用户裁定：tour 滚动走 scrollIntoView block:"center"，锚首卡会滚过头；row-2 首卡居中 = 任何屏高下网格填满视口、钉顶 composer 在顶，相对定位不怕高度差；卡数 <5 步自动跳过；target 串不变 = hash 不变 = 不重放）；**seen 版本 = 内容纯函数**（`lib/tour.ts`：djb2 hash 步骤配置 + EN 副本子树，EN 是 locale 真值源，任何副本改动触发；storage = `localStorage["repurposer-tour-seen"]`；read/write 只在 `useEffect`，禁 SSR）。**任何内容变化触发 Tour 重放一次**——no manual 版本号、no "值得重放"判断。新 tour 沿用同款：独立 storage 键 + 静态 `TourStepDef[]` + EN 副本 hash + `data-tour` 锚点。

#### Composer behavioral contract（意图层单面化：chat 唯一入口）
- **Agent 北极星（ADR-088/089，2026-09-22 拍板）**：Agent continuously produces and refines user-meaningful project artifacts; it does not compile or directly execute workflow internals. Paid execution begins only at the confirmed scope boundary.（Agent 持续生产和修订用户可理解的项目产物；不编译、不直接执行 workflow 内部语言；付费执行只从已确认范围开始。合同全文 = ADR-088/089 + JOURNEYS 旅程四。）
- **Prompt is required**: submitting with an empty prompt is blocked locally (toast), same posture as the auth gate. Files are optional — a prompt-only send creates **no** asset: the intent router judges whether a message IS the user's own content (pasted transcript/draft, with or without "this is my…" framing) and promotes it to a real transcript asset (`create_transcript_asset_from_text`); a generate request with no assets and no pasted content gets an ask-for-material answer, never a groundless plan. Never reintroduce length-based heuristics — content-vs-request is LLM-judged only. Mid-conversation uploads go through the chat dock's attach button — picked files **stage as lifecycle chips inside the input group** (uploading → done/error, × removes and deletes the asset; direct-to-storage, same flow as the composer) and only the **send button** consumes them, riding the turn as message `attachments` (persisted, re-rendered on refresh). Picking a file never sends anything by itself; an attachment-only send is legal — the plan path infers from an honest stand-in line, and a blank message never auto-answers a docked checkpoint.
- **The composer does NO intent recognition**（简报 `docs/archive/tasks-done/intent-surface-unification.md`）: send = spinner (create empty project + upload assets) → navigate to `/projects/$id` — the project page is **canvas + ChatDock, a three-form machine**（ADR-051 条款 7 两态; 2026-09-06 ADR-056 三形态机: pre-first-run the dock is the centered FULLSCREEN chat — message stage fills the page, top-left is just a back pill, canvas not yet shown; the first run's arrival morphs it on one beat — the stage fades out in place (300ms, the old grid-rows collapse read as the chat flying up, user-retired), the canvas fades in on a 150ms delay, back pill crossfades into the full ProjectMenu, and the chat morphs into the DESKTOP panel ("panel" — a FROSTED OVERLAY on the full-bleed canvas in two parking geometries toggled in its header, FLORA "Dock panel" parity: float = right-anchored floating window, vertically inset ~18%/10% (default — FLORA's float is a window, not a full-height sheet), docked = flush right edge full-height, one dock-surface frost in both and the canvas always living beneath it, never an in-flow column; anatomy: slim header (title + geometry toggle + minimize −) / scroller body / bottom register with question pills (the plan confirm pill rides ALL three forms — 2026-09-11 ADR-070 收窄 ADR-063 K5: nobody notices a canvas button at the confirm moment; the dock pill is the confirm beat's ONLY seat — the canvas task_book node is read-face-hidden, one action = one label `generationOverlay.confirm`) + the honesty disclaimer ABOVE the input (2026-09-06 user ruling — FAUNA parity, was below since the 09-02 拆粘) + always-`rounded-xl` input (the folded run-status row RETIRED 2026-09-08, user ruling — the in-flow RunTaskList's one dynamic row is the only progress surface, see CHAT_ARCH §8.7); geometry persists via `localStorage["repurposer-panel-mode"]`, and the page offsets the canvas's zoom pill clear of the panel ONLY in docked geometry (float starts ~18% down and never reaches the corner) via `onPanelStateChange`) or the MOBILE bottom dock ("dock", unchanged); driven behind the page's loading gate by the GRAPH WORLD's arrival — the first run OR (desktop, ADR-057 K5) the docked plan's draft graph (图先展示后运行: the canvas previews the whole chain as draft nodes with per-node quotes before a credit moves; mobile has no canvas and stays run-driven) — so projects WITH runs (or a pending draft) mount straight in panel/dock form and the hydrated first frame never replays. The `?overlay=chat` / `?overlay=run` route params and the fullscreen overlay SHELL stay retired — never reintroduce an overlay route concept; the full form is a layout form of the same dock, not a shell）— with the draft handed over via router state → the chat dock sends it as the first `POST /chat` message (mentions + `persona_id` ride along). **Draft survival across the flip is load-bearing**: one root + an always-rendered card wrapper (`contents` in full/dock, the frosted card in panel) + constant child indexes — the MentionEditor never remounts across a runtime form flip, so its DOM-owned draft + mention chips survive (serialization carry-over is lossy by construction, banned). **Dock visibility = three states**（mobile dock only）: collapsed (resident input group) / expanded (history floats as its OWN frosted layer above the standalone input group — 输入框独立层律, 2026-09-02 拍板) / hidden (user gesture folds the dock to a bottom-right LogoMark dot); **panel visibility = two states**: open / minimized (header − button → the bottom-right LogoMark dot; in float geometry the dot STAYS RESIDENT as a toggle — click parks / recalls by state; in docked it appears only when minimized since the open docked panel's send button owns that corner) — recall triggers are shared (agent speech / pending question), so the user can only hide a STATIC input group, never new information. `POST /chat` is the **only intent surface**: the plan path builds / refines / confirms the plan (the intent router's bounded tool loop, ADR-077); projects with runs go to the chat path's proposal tool set (same loop). Never reintroduce a second intent entry (e.g. a dedicated `/intent` endpoint).
- **clips need media**: enforced server-side — the intent router excludes clips for text-only input; `create_run` mirrors with 422 at the birthplace.
- **SSE 打字机律（绝对规范，2026-09-08 用户拍板）**：agent 的一切言语必须以打字机节奏出现，**整段瞬移永禁**——任何改动不得违背。两条结构牙，缺一条就会出现「thinking 后全量砸下」：① **散文字段入 schema 必带读容忍**（null 读为空默认值——一次 schema 拒收 = 被拒迭代 = 被拒迭代永不流式 = 瞬移）；② **零 delta 路径的最后闸门**（被拒迭代 / 任何非流式兜底）：信封到达后散文照样走 typewriter 节拍释放（`paceSettledProse`），dock 等释放排干才落——散文永远在先、提问随后。新增散文通道 / 改散文字段 / 加提案工具时，两牙同批检查；规格细节 = CHAT_ARCH §8.6。
- **展示文案二源律（ADR-058，2026-09-09 用户拍板）**：一切用户可见的命名类文案只有两个合法来源——① **LLM 建图时命名**（TaskListProposal / WiringProposal / InferredIntent 的 `name` 字段：2-6 词名词短语、界面语言、命名作品不命名工具；null/空 = 未命名，读容忍同打字机律牙①）；② **世界自证**（产物徽标 / 节点状态 / 真实数字——facts 量化摘要与默认值保持确定性）。**冻参模板永禁**（"Post (English)" 式参数拼接标签）——参数 = 不可见执行事实，永不说话；参数一不说话，「文字核对」问题类整体消失。收据 / 收官句的标题 = 生产该 run 的提案的 name（信封时刻从回声行 `intent` 列盖章 `runTitleOverride`；刷新从 `run.context.name` 重建），不读 dock 活 intent；链推导标签收窄为未命名旧数据的回退。**手改撤名**：计划卡任何手改（语言下拉 / count± / bilingual / 增删行）即清空 `name`——提案之名不再为手改后的作品担保，回退读当前参数的诚实标签，下一 LLM 回合再命名；不是对账，是让座。thinking 行 = `chatBusy && !proseActive`（打字机途中不需要 thinking——散文在动即隐藏，排干才回来；`createTypewriter` 第二参的忙/闲边沿驱动，闲态带宽限防抖；**相位帧三拍**（2026-09-10）：understanding → drafting → creating_run，label 恒为当下相位叙事，永冻 "Thinking" = 失信；`previewSeen` 门已删）。**状态行一座两行**：消息流一切瞬态「现在在干什么」行 = 同一个 `components/chat/StatusLine.tsx`（dock thinking 座 text-sm 无读秒 + RunTaskList 动态行座 quiet 带读秒折叠锚），label 恒为当下相位/叙事，永不只是 "Thinking" 冻词。**echo 防编造律（ADR-060，2026-09-10）**：回声只回述用户说过的（作品 / 语言 / 数量），**永不发明平台或场所**（LinkedIn / X / newsletter）——a post is just a post until the user names where it lives；post 类工具/摘要/标签/例句/writer 人设全渠道中立（`linkedin-longform` 仅 pack 内部名，包体中立），渠道的合法来源 = 用户点名或渠道授权，永不是默认值。
...
- **Mentions = 第四载荷字段**（方针 `docs/MENTIONS.md`）：@-entity chips 随首条 chat 消息 `mentions` 字段送达。**注册表架构**（前端 `MENTION_REGISTRY` + 服务端解算）已注册 = `asset`（请求族）/ `output`（指认族，钉 id 服务端确定性解出修订目标，LLM 不猜"第二条"）；候选类型先过 MENTIONS §3 闸门。**配方永不是 mention**——卡面预填模板就是全部发射载荷（配方 = 提示词，ADR-040），服务端永不见 `recipe_id`。**MentionEditor 一家多面**：composer + 持久 chat dock（ADR-041 完成转场后下沉）都挂它；per-surface 候选取流经 `MentionContext`（`components/chat/MentionPicker` fork 已退役）。Input 形态 = contentEditable，chip = `contenteditable=false` inline 节点，DOM 拥有文本，`syncNow` = 唯一同步漏斗；失败轮回滚走 imperative handle（`insertText`/`insertMention`，chip 落尾部）。**Chip 三律**：① 可见（inline + ×）② 发送消费 ③ × 净化（无跨送残留）。**选区引用（ADR-069，2026-09-11——段落级指认）**：文本产物卡（post/article）读体划选段落 → 「引用这段」pill → dock 落带 `quote` 的 @output chip（序列化 = `@label "quote"`；出生截 200 字 / 上下文注 500 字；语义 = 引文即修订范围，agent 改那一段，大屏编辑器路线用户拍板关闭——手动 in-place 编辑只是逃生舱）；`insertMention` 召回隐藏 dock（chip = 新信息）。**失败人话行**：failed 卡读 `spec.error`（sync 时从 failed family step 烤入、家族恢复即清），泛化「运行失败」只是旧行回退。**配方 = preset，never pin**：预填模板只是初稿，全字段可改；面板手编 = 第三方合并 `merge_prior_slots`，**chat 修订恒胜**（chat = 修计划的唯一通道）；卡面 Remix 开检视 overlay（共享 launcher，不自养 form modal）。DAG 用户形态 = FlowView 只读图（`components/flow/`），无 drag/connect/pan/zoom API（结构，非约定）；缩放/平移按面门禁（配方说明书 = 锁 fit，结果画布 = 开）。**画布相机律（C6，2026-09-14）**：聚焦转场只认用户发起节拍——chat 发送后 draft 图到来 = 整链 fit；会话中途新节点诞生 = `setCenter` 平移锁 zoom；**背景 refetch 永不动相机**（2026-08-19 增长不动视口律的收窄不是推翻）；护栏 = 手势防护（3s 盾）/ prefers-reduced-motion 退化瞬移 / docked 面板遮挡补偿 / 布防门（画布已活 + 桌面）；比例尺 pill 带 fit icon。**项目页画布直读持久图**（ADR-057：`GET /projects/{id}/graph` —— 节点 id = 图行 id、边 = 图边、状态 = 行状态，零投影；`runFlow.ts` 与五补丁已火化）；节点族 = **词表 v3**（ADR-076，2026-09-14 三族批 C1~C6）：`type` = 媒介五值 text/table/image/video/audio（`graph_nodes.type` 列，kind→type 已改名；step.kind 工具名不动）+ `spec.prototype` = generator/editor/manual（**程序区按它门控**——editor/manual 卡不渲染 prompt 区，generator 保留，卡面直改暂停 b367e94 不动）+ 业务身份 = `spec.summary` + `spec.tool`；legacy 行（asset/document/generator/processor/agent）永不迁移，读面 `_read_face`（键 = spec.tool）一帧映射，modifier/materialize 两过渡词随旧行存活；task_book 节点读面隐藏（B1-lite，确认拍 = dock pill 唯一座位）；翻译/配音 = **两站**（装配站 video×editor + 文档站伴侣 table×manual，`asm.spec.doc_node_id` 链接，估价 token 段归 doc / units 段归 asm）+ 端口法则（**进 = 消费区域的左下角，一条角律两个区域**：媒体流 video/audio 的消费区域 = 内容区，锚内容区左下角（PROMPT 分界上 16px，底锚上堆）；文本流 text/ctx = 提示词区左下（factsbar 带上）——2026-09-10 红圈裁定，首读「内容区左上」是实现者误读、同日红圈复勘纠正；**出 = 生产区域右上，出锚语义律（ADR-067，2026-09-11）**：出锚 = 节点自身产出媒介（媒介五值直出、text/table 读 prose，素材档案按 asset_type、modifier/materialize 过渡词按 frame_class——一节点一出发锚，出边同点出发），入锚 = 边承载类型；跨媒介边 = 边上转译叙事（video→text），源节点永不长外来 glyph；image/slides 出锚 = 渲染侧第五 glyph（从不过线），ctx 引用流无自有 glyph——入锚并入共享 T（@ 永不上画布，2026-09-12 用户拍板：node 左舷只有 T / image / video 三类 icon）；video/audio/text/ctx 类型化着色；连线 = 圆缘贝塞尔——可见圆即 Handle，xyflow 原生锚 = 矩形位向外缘，切线水平出右缘入左缘）+ caption 右槽恒空（状态原地表达：draft 虚线估价 / running CSS 擦除封顶 96% / done 内容自证 / failed 卡内红 / stale factsbar 徽）+ **卡面交互律（2026-09-13 用户拍板，同日三轮修订）**：factsbar 动作 = copy-or-download + 发布（Send——发布取代删除上栏，产物删除在画布无家；素材条三轮起也只保下载——删除全画布无家，delete API 与 handler 分支同昨日产物例保留），⋯ 菜单全退役；**点击 = 选中 + 右侧档案（OutputInspector，全产物类型）；居中播放器 modal 退役**（项目页 ClipDetailModal 摘除——大屏查看归 hover expand 钮的 lightbox）。**编辑 = chat 或卡面 prompt 直改**（同一个 `edit_prompt` wiring op，**通道分家（ADR-058，2026-09-09）**：chat 修订骑 `POST /chat` 唯一意图面；卡面直改 = 确定性手势不是意图——定价确认住进程序区（锚定受影响子图 = 本节点 ∪ 图边下游、估价随行，动作住节点内 ADR-063——浮卡形态全火化）确认后 ops 由代码构建走 `POST /projects/{id}/graph/revise`，**dock 零消息**（无回声/收据/完成行——loading 在、prompt 变了、产物变了，节点状态周期就是全部反馈），卡面乐观回显（`pendingProgram`）永不回闪；手动布线 UI 永不开放，能力完备手势缺席）。**全文卡律（ADR-063 判词④）+ 封顶滚动律（ADR-067，2026-09-11）**：卡面内容 = 原文全文，永无摘要/浓缩/省略号——DocumentCard 全文渲染 + 卡高封顶 560（`DOCUMENT_MAX_H`），超出正文卡内就地滚动（nowheel+nopan；计划确认拍钉在滚动区之外的卡底，散文永远穿不过按钮）；文档框出生即全文需求高、同值封顶（graph_store `_document_frame` ↔ layout.ts 一条测量律两镜像互引——间距常数同律：`_GAP_MAIN`/`_GAP_CROSS`/`_FRESH_COLUMN_RISE` ↔ `GAP_MAIN`/`GAP_CROSS`，只影响新出生帧；计划文本 = 提案自身的计划散文 `intent.answer`，run fill 只填空面永不改写）；**DocumentCard 最小产物尾（C5）**：`output_ids` 非空 → 卡内底部 factsbar = 版本 pager + copy/download + 开 OutputInspector（尾在卡内，文本锚无外加带）；**估价诚实面**：折叠含未报价节点（transform 链编译期 NULL）时全 NULL 显示「估价随运行」、部分折叠开口「+」——确认拍永不许诺 ≈0。**focus 机制已退役（ADR-058）**：指认产物 = @output mention chip（本段注册表的指认族），请求字段 / 焦点注入 / prompt 规则全删；`messages.focus_output` 读容忍保留（旧行回放灰行前缀），永不新写。**修订 = 图变更**：chat path 的 `edit_graph`（wiring 提案）→ `apply_wiring_ops` 唯一写口 → run 子图填充（下游 = 图边遍历，「Target clip not found」结构性不可能）。
...
- **用户到来即彷徨**（论证 → STRATEGY §5，行为规格 → CHAT_ARCH §3.3）：每步都答「下一步是什么」——开始前配方卡接住 / 计划中确认 dock 接住 / 完成后结果画布闭环接住（ADR-041）。agent 顾问姿态四律：① 诊断一轮封顶【已拍板翻案（ADR-052 判词 5，随 B2 落地改写）：每轮一问、每问可一词答、散文恒带默认路径】（只问用户能答的——听众/目的，参数由配方与默认值吸收）② 带理由纠偏（给替代方案，禁静默拒绝）③ 成功定义随计划 ④ 永给唯一下一步；不做职业/变现咨询（诊断是为了更快给出对方案，不是把生产工具变顾问）。
...
- **Pages under `_app` fill the viewport with `flex-1`, never `min-h-svh` / `min-h-screen`**: `SidebarInset` is a `min-h-svh` flex column (there is no global header, ADR-046) and `flex-1` keeps the page glued to it; taller content scrolls the window. **Exception — home is a fixed app-shell surface**: its root is `h-svh` and never scrolls; ONE internal scrollport (`no-scrollbar`) holds everything — the hero stage, the composer (sticky chrome that parks center-stage at rest and docks at the top as the one-line stadium bar), and the gallery. Any future fixed-surface page follows the same shape: root `h-svh` + exactly one internal scrollport. (The landing page `/` is outside `_app` — its `min-h-svh` is legitimate.)
...
- Structural layout:
  ...
  - **Content**: Flat navigation (no group titles): Home, My projects (`/projects`). ~~Personas (`/personas`)~~ — **hidden 2026-09-23（用户裁定）**，随定位/memory 运营端迭代回归（路由本身存活，只藏入口；composer 的 Persona 钮同批隐藏）。The project grid lives on `/projects`, not on the home page (home is composer-only).
...
- **Border radius**: global small radius (`rounded-md` / `rounded-lg`), avoid `rounded-full` (except for circular icon buttons, red dots, the home composer's docked one-line stadium bar, and the mobile dock's collapsed one-row input group).
...
- **Dot grid = the results-canvas SIGNATURE, one surface only**: only FlowView's `dots` prop carries it — 32px gap + 2px-diameter dot (react-flow `size` is a diameter) + muted-foreground 32% light / 30% dark, all FLORA's measured world constants (2026-09-02); plain world-space, scales with zoom (no re-tiling, never re-add zoom compensation). **The home page dropped the grid 2026-09-02 (user-ruled)**: a fixed texture behind a non-pannable surface advertises an affordance that isn't there, and single-surface use makes the dots MEAN "you've entered the graph" — home is pure gray-ladder elevation. There is no `dot-grid` CSS utility anymore; any second dotted surface is a violation (ADR-046 附).
