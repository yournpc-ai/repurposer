...
- **SSE 打字机律（绝对规范，2026-09-08 用户拍板）**：agent 的一切言语必须以打字机节奏出现，**整段瞬移永禁**——任何改动不得违背。三条结构牙，缺一条就会出现「thinking 后全量砸下」：① **散文字段入 schema 必带读容忍**（null 读为空默认值——一次 schema 拒收 = funnel 修复轮 = 永不流式 = 瞬移）；② **extractor 的 null 值只跳过该对**（多键同监时，一个键的 null 绝不锁死另一个键的散文——ask 判决恒带 `"answer": null`）；③ **零 delta 路径的最后闸门**（funnel 修复轮 / 任何非流式兜底）：信封到达后散文照样走 typewriter 节拍释放（`paceSettledProse`），dock 等释放排干才落——散文永远在先、提问随后。新增散文通道 / 改散文字段 / 加提案态时，三牙同批检查；规格细节 = CHAT_ARCH §8.6。
...
- **用户到来即彷徨**（论证 → STRATEGY §5，行为规格 → CHAT_ARCH §3.3）：每步都答「下一步是什么」——开始前配方卡接住 / 计划中确认 dock 接住 / 完成后结果画布闭环接住（ADR-041）。agent 顾问姿态四律：① 诊断一轮封顶【已拍板翻案（ADR-052 判词 5，随 B2 落地改写）：每轮一问、每问可一词答、散文恒带默认路径】（只问用户能答的——听众/目的，参数由配方与默认值吸收）② 带理由纠偏（给替代方案，禁静默拒绝）③ 成功定义随任务书 ④ 永给唯一下一步；不做职业/变现咨询（诊断是为了更快给出对方案，不是把生产工具变顾问）。
