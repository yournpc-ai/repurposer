...
| LLM provider 抽象 | P1 | ADR-025 已决（薄接口）未实施；触发 = EU 客户要求 Mistral/EU-hosted | 2027 架构（MODULE_ARCH §4.1）。UX 形态定调（2026-08-02）：用户-facing 是**策略开关**（如"优先 EU 托管模型"），不是模型 SKU 货架（对照 Lovart 模型选择器，否决）；composer 模型 pill 已改 hover 信息卡（只读展示当前 provider 分解）。实施第一纪律 = **边界规范化**（dsh defensive-patterns「公共约定两侧都要遵守」：provider 的多种失败形态在边界单态暴露，消费方永不猜异常来源，`research/deepseek-harness.md` §3） |
...
| 命名对齐批 v2（interrupt + registry + skills→tools 换位 + providers/llm） | P2 | 无（判例 N-40/N-41/N-42 已立） | 2026-08-20/21 行业坐标审计 + 四厂商规范证据（`research/agent-skills-spec.md`）：① 节点 kind `checkpoint` → `interrupt`（kind 字符串 + alembic schema 迁移 + `expire_stale_checkpoints` 函数族 + `dock_checkpoint_question` 等引用面）；② `agents/roster.py` → `agents/registry.py`（import 面）；③ `app/skills/` 能力包 → `app/tools/`（TOOL_REGISTRY；TaskItem.skill→tool 全量换名，**存量数据全清（reset_db），零容忍 shim**；plan/intent prompt 目录措辞同步换，先 stash 零假设测试）；④ 原 `app/tools/` 拆 `app/providers/`（asr/voice/vision/storage）+ 通用件随消费方（dubbing→dub 包 / filler→filler 包 / transcript+extraction+music 按 import 图）；⑤ NAMING 词汇表五行重写 + §1 营销泛词豁免条款（已落文档层）；⑥ `clients/` → `providers/llm/`（Model 缝，含 PRICING 价目表；第二 provider 的家，ADR-025 薄接口落地处）。六项同批做，词汇表行同步去「更名中」标记 |
| 默认指令包最小垂直切片（架构验证） | P1 | 命名对齐批 v2（`app/skills/` 座位空出） | 2026-08-21 拍板「必须弄点默认指令包证明架构流程」：pack 规范 = SKILL.md（Agno 六键 frontmatter）+ loader + **SKILL_REGISTRY**（行业本义重生——能力侧已让位 TOOL_REGISTRY）；首包 `linkedin-longform` 从 write_post 内嵌 prompt **逐字节平移**（知识搬家、一字不改）；消费 = write_post 声明 packs 装配期注入（模型无感，无 runtime discovery）；覆盖 = name-wins 整包替换（persona 级下批）；验证 = 零假设（平移前后产物等价）+ 剧本回归 |
| 执行中自适应重规划（interrupt + 重 authoring） | P1 | interrupt 节点（✅）+ 任务书 refine 链（✅）；与素材理解前移（上方 P2 行）互补，agent 调用台账（上方 P1 行）供取证 | 2026-08-20 拍板必做：run 中途节点产出的事实与任务书假设矛盾（ASR 检出素材语言 ≠ 任务书语言、素材内容与预期不符等）→ 自动 interrupt + 模型重 authoring 一版任务书，diff 确认后续跑——人在环重规划，**非 tool-loop**（常备否决不翻案）；预计 2–3 天，排期窗口待指认（建议第九周台账前置之后；插入十二周内需指明替换项） |
