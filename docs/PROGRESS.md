...
| 精确编辑迭代（edit_output + quote→range + work/version 归档 + 计费偏好集成） | P1 | 修红批（✅ 2026-09-24 `6b93b24`）；零探索退化形态行（edit_graph 退役弧）可并行不阻塞 | **2026-09-24 拍板（ADR-090/091/092，同批落档）**：年底审计判定精确编辑 = 距最大公约数唯一缺口——机制全在而 LLM 不可见（21 ops llm_visible=False）；live 验收野外实证（改字幕色只能整条重生成）。四件一迭代：edit_output 受控终态工具（MVP 四件 enum）→ quote→range 确定性解算 → 重跑归档不变量（交付后产物历史不可销毁只可归档）→ 计费偏好真生效（集成不重设计）。排期窗口待指认；施工简报 = `tasks/precise-editing-iter.md`（冻结条 9 件 / E1~E6 全带推荐答案 / 切片 S0~S6） |
