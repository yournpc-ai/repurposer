...
| 一 08-10 | 品牌设置并入人设：字体、颜色、logo、字幕样式从此跟随人设，"品牌模板"概念退役，首页身份选择收敛为一个控件；图片视频卡的示例素材入库，"点卡 → 上传 → 发送 → 对话接住"入口链全程走查。架构规范化动工：各项能力（剪辑/配音/字幕/配乐……）开始归拢为独立的"技能包"；同日 P2 落地——生成步骤全部对象化为节点（重试预算 / 输入门禁 / 展示名收归节点自描述），产物类型由注册表单一派生（新增一种内容形态 = 一条注册项，当轮生效），存量步骤数据自动更名平移；剧本回归（S1–S40 子集）与真实生成链路实测双绿。同日 P3 提前两天落地（5533b18）：所有 AI 调用统一经过"校验 + 结构化回显修复一轮"漏斗（盲重试清零、兜底改声明制），chat 上下文装配收归统一装配层（`agents/contexts.py`），计划/意图两个 agent 归一为流式声明实例；新增"repair 只一轮"剧本断言（S41），回归 17 项全绿 + 真实生成链路实测绿 | 设置风格只剩一个去处，首页少一个控件；从点卡到进对话全程无断点。对交付速度：此后新增一种内容形态 = 加一个技能包，这是"每周一张新卡"的前提 |
