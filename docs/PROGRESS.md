...
| 08-10 | **排期重排（用户拍板）**：架构迭代（ADR-039）、意图层收尾（轨 A 顾问姿态）、配方卡闭环链三线并入本周（08-09~08-14）与人设模块并行收口，周五 08-14 联合验收；后续整体提前 6 周——分镜 08-17 起、四卡齐亮 09-04、商业化闭环 09-18、分发+合规 09-25、go/no-go **10-02**（回退 10-09） |

---

## 2. 后续排期开发计划（2026-08-03 → 2026-10-02，仅工作日）

**排期口径**：工作日顺排，每周五验收并滚动修正后续日程；2026-08-10 起重排——架构迭代 / 意图层收尾 / 配方卡闭环链三线并入第二周收口（见 §1.3 本日行），后续周次整体提前。
...
| 第三周 | 扩展配方类型：分镜剪辑（⚠️ 压缩 1 周，回退吃第四周缓冲） | 三卡齐亮 |
| 第四周 | 扩展配方类型：AI 虚拟画面（上）——声纹打磨 + R5 提前量 | — |
| 第五周 | 扩展配方类型：AI 虚拟画面（下）——Memory + 门禁 v2 + R5 卡 | 生成产物线就绪（四卡齐亮） |
| 第六周 | 成本统计深化 + 生成前费用预估呈现 + 失败不扣费 + 定价设计 | 成本透明 + 定价就绪 |
| 第七周 | 支付接入 + 套餐权益 + 用户计费中心 | 商业化闭环 |
| 第八周 | 双平台联调 + AI 内容标识 + 披露 | 分发 + 合规就绪 |
...
**外部因素（需按期启动）**：LinkedIn / TikTok 开发者权限（暂缓，时间未定——2026-08-05 拍板；第八周联调排期届时按实际申请时间重排）；支付商入驻申请（08-14 前提交，审批周期数周）；律师法务联系（08-31 前启动，周期 2–4 周）。术语表、管理后台、帮助中心等最后项按需再考虑。
...
> ⚠️ 压缩为 1 周：方案已收窄为静态双人分镜（RECIPES §6），且闭环链之后卡面内容 = 数据条目 authoring，不再占工期。回退：滑期吃第四周缓冲（08-28），go/no-go 最坏移至 10-09。

| 日 | 交付 | 验收口径（用户视角） |
|---|---|---|
| 一 08-17 | 立项评审（重启 VIDEO_EDITOR 封存的 auto face reframe 决策评估）+ "识别谁在说话"技术验证（whisper 词轴切段 + M3 画面网格判定，`demo/uploads/xy_1.mp4` 已策展） | 立项结论落档；系统能自动分辨访谈中谁在说话 |
| 二 08-18 | `speaker_map` 分析节点 + `crop_track` 关键帧轨契约（preview=render 同源） | 说话人时间轴落成可重跑的数据 |
| 三 08-19 | 防眩晕平滑（最短驻留 + 缓动）+ `reframe_clip` 过 NAMING §7 评审登记 + 对话可调用 | 对 AI 说"把这条换成分镜模式"就变 |
...
### 第四周（08-24 ~ 08-28）：扩展配方类型：AI 虚拟画面（上）——声纹打磨 + R5 提前量

> 人设模块（皮肤吸收 / 声纹缓存 / 显化页 / 触点 / 门禁 v1）已于第二周落地；本周为声纹质量 + R5 前期。

| 日 | 交付 | 验收口径（用户视角） |
|---|---|---|
| 一 08-24 | 声纹合成质量与稳定性打磨 | 生成的语音像本人在讲，不机械、不出戏 |
| 二 08-25 | 声纹合成续 + 生成链路回归测试 | 生成过程稳定，不失败、不用返工重来 |
| 三 08-26 | R5 提前量（`voice_gen` / `synth_visual`，简报 `tasks/synthetic-talk-video.md`） | 把资料给它，开始能生成"我的视频" |
| 四 08-27 | R5 提前量续 / 缓冲 | — |
| 五 08-28 | 【验收+缓冲】进度回填（兼分镜线滑期回退位） | — |

### 第五周（08-31 ~ 09-04）：扩展配方类型：AI 虚拟画面（下）——Memory + R5 卡
...
| 日 | 交付 | 验收口径（用户视角） |
|---|---|---|
| 一 09-07 | DAG 成本统计深化：按节点 / 按产物 / 按 run 的费用视图 | 每次生成具体花在哪一步，一目了然 |
| 二 09-08 | **生成前费用预估呈现**（架构迭代 P4 估价地基已于第二周落位，本周接消费面）：dock 生成前总价 + chat 修改单价 + 配方卡估价贴（报价 = 图 fold） | 点"生成"之前，先看到这次大约花多少 |
| 三 09-09 | actual 校准闭环雏形（estimate vs cost 偏差回归，收窄报价区间）+ 失败不扣费语义 | 报价越用越准；生成失败不花钱，不用为系统问题买单 |
| 四 09-10 | 定价调研与套餐设计（"一场演讲 = 一套内容包"） | 价格结构简单可预期：按"一场演讲一套内容"算账，不是看不懂的积分 |
...
| 日 | 交付 | 验收口径（用户视角） |
|---|---|---|
| 一 09-14 | 支付接入（上）：供应商对接与订阅创建 | 可以在线订阅付费，流程顺滑 |
| 二 09-15 | 支付接入（中）：订阅生命周期（续费 / 升级 / 取消） | 升级、续费、取消都自助完成，不用找客服 |
| 三 09-16 | 套餐权益执行：额度检查 / 超限提示 / 周期重置 | 自己套餐含多少、用了多少、什么时候重置，清清楚楚 |
| 四 09-17 | 用户计费中心：用量、明细、套餐页入口 | 每笔消费有据可查：什么时候生成了什么、花了多少 |
| 五 09-18 | 【验收+缓冲】支付端到端联调 → 🎯 **商业化闭环** | 从免费到付费的完整体验：订阅、使用、查账、管理套餐 |
...
| 三 09-23 | AI 内容标识：技术选型 + 自动判定（全 AI 合成必标 / 纯剪辑豁免） | 该标的内容自动标，用户不用自己判断和勾选 |
...
| 四 10-01 | 全周期验收：九个阶段成果整体走查 | 上线前完整体检 |
...
| 目标上线日（2026-08-10 重排：go/no-go = **10-02**——架构迭代与闭环链提前收口，后续整体提前 6 周；回退方案 = 移至 10-09） | 总监 |
| 律师人选与预算 | 总监 |
