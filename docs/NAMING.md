...
| 配方发射身份 | `recipe_id` | 配方身份的首发消息载荷（plan-path transport，`persona_id` 同座位：carry only, never persisted）；点卡 = 唯一来源，播种唯一发生地 = 服务端 `resolve_recipe_launch` 按它查注册表 | 不是 mention、不进 prompt 文本、不开第二通道（项目列 / run 配置） |
| 配方播种 | `resolve_recipe_launch` | 服务端预设播种函数（`pipeline/recipes.py`）：`recipe_id` → 注册表查条 + live 校验（unknown/reserved → 422，LLM 前 fail-fast），存在性填充 + `merge_prior_slots` 三方合并 | 不是 LLM 从 prompt 推断、不是前端 prior |
...
| N-25 | 任务书预设归服务端配方注册表解析；mention 系统双端注册表化 | 预设播种唯一发生地 = 服务端 `resolve_recipe_launch`（`recipe_id` 首发载荷，composer/overlay 两表面同一通道）：存在性填充（只补推断没有的槽位类型）+ dub 空时填默认，无 explicit，每个字段可经 chat 修订（chat 永远赢，三方合并 `merge_prior_slots`）；客户端 prior 构造路径禁建。提及类型与效果各自注册表化（asset 为成员；recipe 已退役为 `recipe_id` 载荷——配方 = 发射上下文，MENTIONS §3），后续 @ 类型只填注册项，禁类型分支补丁；"硬编码"表述禁——正确表述是"静态注册表，随代码部署"（SKILL_REGISTRY 同款纪律） | §1、§5 |
