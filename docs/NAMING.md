...
| N-59 | 精确编辑迭代词族：`edit_output` / `quote→range` 解算器 / `work_id`+`archived_at` / 归档不变量 / `confirm_strategy` | 迭代拍板项（2026-09-24 词表先行，ADR-090/091/092 + 简报 `tasks/precise-editing-iter.md`）：① **`edit_output`** = 精确编辑受控终态工具（chat path 终态动词族第五词，propose_*/revise_* 同族）：args = `{target:{output_id?, plan_ref?}, kind, params:{quote?, seconds?, style?, title?}}`，指认二选一、全空 = ask 反问；**`kind` enum MVP 四件 = `remove_range` / `set_trim` / `set_caption_style` / `set_title`**（21 原始 ops 永不进 LLM 词表）；与 revise_output 分工判据 = 变更是否可机械执行，判不准 = 域拒绝回环；② **`quote→range` 解算器** = `locate_span` 升格的 edit 面确定性服务（LLM 只给 quote 文本，range 归代码解算；置信谱：数值引用 > marker 匹配 > 全文唯一 > 多义/未命中 = 域拒绝回环）；③ **work/version 两身份**（ADR-091）：**`outputs.work_id`** = work 物理锚（出生新 UUID、重跑/修订重建继承 doomed 行）+ **`outputs.archived_at`**（可空时间戳律，NULL = active；读面默认 `archived_at IS NULL` 过滤——`outputs.status` 既有列是 generated 遗迹，不复用，S1 施工修正）；④ **归档不变量** = 「交付后的产物历史不可销毁，只可归档」——wipe 点（select_clips wipe / derivative sweep）只准写 `archived_at`，verify 回退保持物理删除（生产中途例外）；`restore_version` = 同 work 内 archived_at ↔ NULL 换态（至多一 active）；⑤ **`confirm_strategy`** ∈ `always` / `large` / `never`（ADR-092）：users.settings JSONB 键 + configs `billing.confirm_large_threshold` 参数；确认闸形态 = plan dock 披露强度分级 + CTA 显价（零新增手势） | §1、§6 |
