...
└───────┬──────────────────────────────┬─────────────────────┘
        ▼                              ▼
┌───────────────────┐   ┌─────────── Pipeline ✅（RunPlan 内核）────────┐
│ Operation Model ✅ │   │ 摄入/预处理（ASR）✅ │ 两步/agent 花名册 ✅    │
│ （操作日志层，      │   │ ┌── RunPlan 内核（施工图，ADR-028 ✅）───┐│
│  三前端共用）      │   │ │ workflow_steps：前奏两步/技能节点/质检节点   ││
...
│ 正向边：注入 plan 前奏 / chat / 分发调性（✅ 单向，规则 4）  │
└────────────────────────────────────────────────────────┘
```
...
| Guardrails | 📋 合规底座（ADR-026 分类器/C2PA） | PROGRESS 第十一周 |
