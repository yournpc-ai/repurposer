...
- **对我们的冲击与应对**：
  ...
  - 需跟踪：其配音质量是否过"审校线"~~、是否支持术语表（暂未见）~~ → **2026-07-28 闭环**：Agent Opus 配音确认屏已有 Pronunciation 自定义（发音级词条，"+" 内联添加入口）；对我们的启发 = 术语表一份资产两个消费者（固定译法喂翻译、发音喂 dub），且词条入口长在用得到的地方。审校线质量仍未实测。
...
- **摄入的目标形态**："接管源后持续自动"（auto import 新内容），不是"手动贴链接"——校准我们 PROGRESS 链接摄入线 链接摄入的终态。
...
- **计费颗粒度到动作**：生成按钮即标价（20⚡），按 part 重生成（Regenerate part 20⚡）——成本可见已不是独有叙事，我们的"生成前预估"必须提速（PROGRESS 成本预估线）。
