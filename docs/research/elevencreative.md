...
**对我们的意义**：合规从"安全页徽章"升级为"定价页上有名字的开关"——正是 STRATEGY §2.3"信任当产品做"的呈现野心参照。HIPAA 与我们无关（医疗）；residency / zero-retention / 审计日志分别对应 PROGRESS 合规线 各行。
...
| 合规具名 SKU | PROGRESS 合规线 各行的呈现形态（STRATEGY §2.3） |
...
## 8. 二轮补充与修正（2026-07-22）

**机制 5：子图运行菜单 + 节点级积分预览**（截图实证）：节点运行菜单提供「从这里运行」（本节点+下游闭包，示例 3,200 积分/2 节点）与「运行到这里」（本节点+上游闭包，示例 9,012 积分/5 节点）——部分执行的用户词汇 + **执行前成本预览**，对手已到子图级（对应我们 PROGRESS 成本预估线 成本预估、§3 子图词汇：只跑此节点/从这里跑/跑到这里）。

**机制 6：节点动词全集 + 对象↔chat 双向引用**：节点右键菜单覆盖全生命周期（运行/重命名/复制粘贴/删除）、产物动作（下载/保存到资产/提取生成/删除生成项）、图操作（设定开始帧/聚焦/**标记模板输出**）、chat 桥（**在智能体聊天中引用**）；流程 chat（流程智能体，alpha）内 @ 唤起类型化资产选择器（图像/视频带缩略图），agent 消息带 👍👎 反馈，输入框带"自动低于300积分"**预算帽路由**下拉。可吸收：步骤清单节点卡片的动词集、chat 的 @ 类型化引用模型（CHAT_ARCHITECTURE 核心语义）、配方声明交付物节点、预算帽（BILLING 参照）。

**机制 7：源节点是一等公民**（修正 §3/§6 表述）：上传素材是带完整菜单的源节点（从这里运行 / 替换资产 / **标记模板输入**）。准确分界：Flows 有 **source-of-record**（素材作为引用/起点），没有 **surgery-on-record**（对素材时间轴做选段/trim/隐藏的手术节点）——后者是我们节点族的职责（选段/渲染），DAG 代数完全容纳，但它的节点族不含这一类。

**workshop/ledger 准确化**（修正首轮"workshop 成本记账弱"的过强表述）：机制 5 证明 workshop 模型也能逐节点记账。两模型的真实差别在**图可变**：run 之后改图，图就不再代表产出第 N 次结果时的状态——除非每次运行都快照参数（未知）。我们的 per-run 冻结图是"产出瞬间状态永远可答"的最自然实现（机构审计 / C2PA provenance / persona 校准的硬需求），这是他们可补、我们免费的差别，不是能做不能做的差别。
