...
| 机制 | 我们的翻译 |
|---|---|
| DAG 作为生成表征 | **内化**：run graph 持久化（ADR-028），agent 编排，用户看步骤清单 |
...
**机制 5：子图运行菜单 + 节点级积分预览**（截图实证）：节点运行菜单提供「从这里运行」（本节点+下游闭包，示例 3,200 积分/2 节点）与「运行到这里」（本节点+上游闭包，示例 9,012 积分/5 节点）——部分执行的用户词汇 + **执行前成本预览**，对手已到子图级（对应我们 ROADMAP §8 成本预估、§3 子图词汇：只跑此节点/从这里跑/跑到这里）。
