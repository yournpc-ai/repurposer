...
工作流内部**永远函数名**（`select_clips` / `dub_clip` / `write_post`——动词族零人格）；用户可见的**进行态叙事**是节点声明上的可选展示属性（`task_name` 机制的升级位）：「正在剪辑成片…」「正在制作配乐…」渲染在打勾流 shimmer 行 / 图节点卡（running 擦除原地表达，ADR-057）。
