...
| 资产 | 一句话 | 我们的载体 |
|---|---|---|
| **品味** | 知道哪 30 秒值得剪 | Content Director + 传播潜力分 + Gallery（§5） |
...
| L2 质量控制 | 分数落库+门槛+理由；persona 保真可验；术语表；节奏/运镜枚举 | ❌ 分数只写日志；clip-spec 无 motion 原语（crop 整条静态） |
| L3 寻址控制 | 步骤级重跑、chat 可寻址、run plan 可检视 | 🚧 仅按输出类型重生成（粗粒度） |
| L4 交付控制 | 分发、定时、回流 | ❌（设计完成，DISTRIBUTION.md） |

**排期含义**：配方作为产品 = L1+L2 兑现之后——否则配方卡是营销剧场。P0 集群（计量钩子 / 分数落库 / 去静默 / AI 标识）全部在 L2 路径上；L1 与 L3 的共同地基是 RunPlan 持久化（ADR-028，ROADMAP §1）。Gallery Phase 1 硬编码卡不受此限（获客面，手工策展即可）；但"agent 兑现配方"必须排在控制深度行之后。
...
| 本文条目 | ROADMAP 落点 |
|---|---|
| §2.1 品味可见可证伪 | §1 传播分持久化（P0-3）/ 打分理由（P1）/ persona 校准（P1）；§2 Operation Model |
...
| §3 牌 1 工作流闭环 | §1 链接摄入（P1）；§5 Distribution 全节 |
