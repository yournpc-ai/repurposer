...
**Decision**:
1. **静态配方流程图 = 采纳**。配方注册时作者策展的只读结构图（友好步骤名、固定结构、无模型名、不可接线），作为"它是怎么做的"堆叠项住进配方检视 overlay（W4 轨 B）。定位 = 说明书/信任件，回答"它拿我的素材做了什么"，不承担任何操作。
...
3. **运行期活图（故事地图）= 证据裁决**。W7 工作面周做 spike：复用第 1 条的静态渲染器（同画笔，数据源换活 `workflow_steps`），只读投影、只画当前 run + 最新产物（历史归消息流记录卡）、默认开给配方 run。W8 周五以**小白复述测试**（看完能用自己的话讲出"它拿我的素材做了什么"）+ 使用数据裁决：升正为默认中心，或留检视模式。ADR-028 的"用户侧永不见 DAG 画布"自此修订为"用户侧永不见**可操作**画布"。
...
**Consequences**:
- CLAUDE.md 作曲家契约行"the DAG is never user-facing"随 W4 配方 overlay 落地时修订为"the operable DAG canvas is never user-facing; the static recipe flow is its only user-facing form"。
- 第 7 周原"DAG 编辑 go/no-go"改为第 8 周周五"活图展示 go/no-go"，范围大缩：不评估编辑，只评估展示。
