...
> 准绳：`docs/archive/tasks-done/runplan-persistence.md`（简报）；决策：ADR-028/029/030；概念：`docs/AGENT_ARCHITECTURE.md` §12
...
- `docs/archive/tasks-done/runplan-persistence.md` status → Implemented；§4.4 口径同步为"3 物理位覆盖 4 逻辑入口，derivative_dispatch 不建 run"。
- 不需要新 ADR。
