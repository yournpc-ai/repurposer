...
| `RECIPES.md` | 配方架构母文档：home 能力演示卡 + 兑现管线（caption catalog / dub 接线 / voice_gen / 分镜指引）+ R1–R4 分期 | 🚧 R1/R2 已落地（caption catalog + stacking + dub 接线 + 卡片层 + align_stills 无声图片视频；R3–R4 待施工）；Remix = overlay 内发射 + `recipe_id` 载荷（MENTIONS §3，简报 `tasks/recipe-launch-context.md`） | 动首页配方卡、字幕样式、dub/合成视频/分镜能力前；配方线 tasks 简报的母文档 |
