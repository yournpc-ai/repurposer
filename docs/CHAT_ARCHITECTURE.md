...
- **碎碎念一行 = Claude Code 式 status line**：单行、live（"Thinking for 37s…"）或回溯，说**现在正在发生什么**（shimmer + 阶段自身耗时）；节点行说 **WHAT**、碎碎念说 **NOW**，两行永不重话。dock 的 ThinkingRow 同构：相位标签 + 客户端 elapsed ticker。
