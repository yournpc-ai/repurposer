...
- `ask` → **一等动作直通 dock 提问机器**（ADR-052 B2 D2-C1，caption-mode 特例泛化为正典）：`_dock_question` 选项问形态，payload 带 `slot` 握手（作答由 autoResume 选项命中或 slot 握手判定回填账本槽位 user-stated，回 book path 重判——ADR-053 R2）+ `default_path` 牙齿（dock 散文第二句——每个问题都可安全跳过；选项问 bail + slot 以替身行恢复 book turn，走到默认路径出书）。账本合并落 ledger-only 行（`intent=None`，桌上存书原样保留）。同槽位重问被代码翻回 draft——问环有界
...
- **停靠法则（形态律，ADR-053 R1）**：渲染按 `options` 是否为空分流——**文字问**（options 空）= 普通对话消息，直接入消息流，永不 dock、永不等答（待决行住服务端，后续回合带来判定结算或提醒尾）；**选项问**（options 非空）= input 正上方的**非阻塞浮层 pill**（2026-09-02 ADR-051 条款 8 拆粘：独立浮层、小间距、恒可见不随计划卡滚走；问句 + × + 选项行 + default_path 散文行）。**输入框永不因提问隐藏**——阻塞 morph（2026-08-31 形态切换）与尾行铅笔手输入同批拆除，永不回归：自由回答就是输入框自己的发送。× 保留但不是"阻塞态的出口"而是**显式跳过**——走问题的 default_path，interrupt 的 × 停掉在跑的付费 run。**同一时间最多一个待决**——新题落库前旧题 auto-bail（`answer.text="superseded"` 机器标记）。回答瞬间坍缩成**已答问题双层消息**入流（`AnsweredQuestion`——只对选项问与 task_book 回执；文字问的回执 = 普通消息对）。
