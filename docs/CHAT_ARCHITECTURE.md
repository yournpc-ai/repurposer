...
- `ask` → **一等动作直通 dock 提问机器**（ADR-052 B2 D2-C1，caption-mode 特例泛化为正典）：`_dock_question` 选项问形态，payload 带 `slot` 握手（作答由 autoResume 选项命中或 slot 握手判定回填账本槽位 user-stated，回 book path 重判——ADR-053 R2）+ `default_path`（跳过即走默认路径出书——× / bail 取它、提醒尾消费它；2026-09-04 起不再直渲为 dock 行——用户拍板：Opus/FLORA 均无此行，正常对话即可；选项问 bail + slot 以替身行恢复 book turn）。账本合并落 ledger-only 行（`intent=None`，桌上存书原样保留）。同槽位重问被代码翻回 draft——问环有界
...
- **前端**：`lib/chat-stream.ts`（fetchEventSource POST，**禁自动重连**——重试会重复落用户消息；非 2xx 读 JSON detail 保持 422 toast 语义）。两个面同款节奏：发送 → thinking → 首个 delta 才建预览气泡（Streamdown `mode="streaming"` 渲染不完整 markdown）→ 信封**原地落定**（同 key finalize 转静态，**永不 remount**——删除重推会让已定散文重播入场动画 = 肉眼闪烁；answer 路径 2026-09-06 起同纪律：乐观 QA 块**原位**替换为归档行、预览泡原地转静态，follow_up 散文由预览携带 echoCarried 不重推）+ 跑现有完成逻辑。**thinking 行只覆盖 send → 首个 delta**：预览泡一旦存在过本回合永不回归——`previewSeen` 门（2026-09-06 首回合闪烁根修；settle 窗口（出书路径 `await fetchPendingBrief`）chatBusy 仍真且无 streaming 消息，`!streaming` 派生门会在已完成文本下方闪回 thinking 行）。静态历史消息同走 Streamdown（`mode="static"`）。
...
### 8.7 RunTaskList 显示律（2026-09-04，chat-flow-sequencing 验收 5；Claude Code status line 解剖为用户参照）

- **Header = 计划标题 + 总耗时**。静息标题，**永不显示 `running?.summary`**——「当前活动」归碎碎念一行独占；头部劫持 running 名 = 三行同指一节点的三重冗余（头部 / 碎碎念 / 行 subtitle），否决。
- **碎碎念一行 = Claude Code 式 status line**：单行、live（"Thinking for 37s…"）或回溯，说**现在正在发生什么**（shimmer + 阶段自身耗时）；节点行说 **WHAT**、碎碎念说 **NOW**，两行永不重话。dock 的 ThinkingRow 同构但**无读秒**（2026-09-06 用户拍板：相位标签 shimmer only，"· 3s" 退役——消息流里的思考行不竞赛计时；run 级时钟只在 RunTaskList 表头/收据）。
...
- **表头即锚 + 步骤平铺（2026-09-05 层级裁决，翻案 09-04 折组）**：`☷ 标题 · 总耗时` **自己就是 expand/collapse 开关**（CC 的 `* Creating… (2m 45s)` 行），步骤在其下**平铺**——「Preparation · n 个步骤」组行是假中间层，连同自动展开三条例行逻辑（组内活时展开 / 手切尊重 / 防孤儿强制展开）整体退役。长任务保留清单（学的 coding agent）；短单个操作 = FLORA 式动态动作行（ThinkingRow / 碎碎念 / RunStatusRow 三化身，清单外不存在）。
