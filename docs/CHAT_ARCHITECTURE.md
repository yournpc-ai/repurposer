...
**前端实现**：`useRunEvents` hook 统一消费这条流，接项目页 dock 打勾流（2026-08-31 ADR-051：fullscreen 壳退役、dock 壳唯一形态；计划卡 HITL 确认 → 打勾流入流——默认展开的活清单钉消息流底，终态收成一行收据；输入组上方的折叠打勾行 2026-09-08 用户拍板退役，§8.7；run 期画布 = 持久图节点原地填充（ADR-057：图先展示后运行——dock 即 stamp 草稿图，run 把节点 queued→running 擦除→产物落地反写节点产物区，画布直读零投影；**节点态的 run 期活读 = 同一 SSE 的 step diff 驱动 graph refetch**，step 每次状态翻转都伴随节点反写，画布与清单同一拍））。轮询只保留给无 token 的匿名场景与"run 已终态但 clip 仍在渲染"的尾部阶段。
...
- **终态 chrome 降灰（2026-09-05 验收拍板，CC 归档灰阶；09-08 收窄为同 register）**：收据行 = 动态行同款的 text-xs muted（✓ 标题 · 总耗时）——归档元素是事实收据不是对话正文，灰 = 耳语不是换字号换字重；**收官散文是普通回复消息**，恒走消息本体样式（它不是 chrome 不收特殊样式），走 AssistantText 同一管道（Streamdown，与每条 assistant 散文同一渲染器——裸 `<p>` 特殊管道已退役）。**终态 header unit 退役**：起始 banner / QA stand-in 是活相 chrome，terminal 时收据即归档头。失败 run 例外：清单下保留人话失败行。
...
- **禁止**一个 run 在消息流里开两个渲染面（**单一渲染面律**，2026-09-10 用户实拍「消息错乱」取证）：dock 已 attach 的 run 只经 runStreamUnits 渲染（活态打勾流 → 终态收据，锚在 run 时序上）；生产行的 `workflow_run_id` 章是**排序锚不是渲染触发**——书行章曾让同一 run 内联 RunCard 在用户开始消息**上方**渲染、units 在下方再渲染一遍（两面两 SSE 订阅两快照）。detached（旧史不再 attach）的 run 才走内联 RunCard——那是它们唯一的面。
