...
- **动态行 = 全部 chrome（2026-09-08 用户两轮拍板——先合并、再纠正灰阶）**：打勾流 = 一行动态动作行 + 其下平铺步骤，没有第二层，**全程同一耳语灰阶（text-xs muted）**。**运行中**它是 CC 式 status line（● shimmer 碎碎念 = **现在正在做什么**——行文本永远动态 + 阶段自身耗时）**且自己就是 expand/collapse 开关**；**终态**是**同一行的收据形态**（✓ 计划标题 · 总耗时 · chevron——同字号同灰，ADR-057 §9 动作行一句话原位变化，不叠行）。被删的「☷ 标题」表头行（text-sm/font-medium/ListChecks）是错规格的第二层，**不是收据的合法形态**——标题归收据行、当前活动归动态行、总耗时归收据行，表头一无所有。节点行说 **WHAT**、动态行说 **NOW**，永不重话。dock 的 ThinkingRow **同一组件但无读秒**（2026-09-09 StatusLine 一座两行，见 §8.6；2026-09-06 用户拍板：相位标签 shimmer only——消息流里的思考行不竞赛计时；run 级时钟只在动态行/收据）。**RunStatusRow（输入组上方折叠打勾行）同批退役**：三形态机下消息流恒在（panel 滚动区常驻 / dock 历史一拍出），折叠行与碎碎念逐字重复——进度面 = 流内打勾流一处，唯一进度面律更纯；短单个操作同样由动态行承载（FLORA 式动作行与清单不再分家）。
