...
- **协议**：`assistant.delta` `{"text"}`（0..N，有序拼接的散文预览）；非散文片段（think 前缀 / verdict JSON 尾部 / reasoning）推 `assistant.thinking` 帧——两种载荷分流：**`{}` = 纯 keepalive**（只保温 thinking 指示，永不展示，语义不变）；**`{"phase": …}` = 真实相位切换标签**（chat-flow-sequencing C，2026-09-04：唯一标签 = start 分支起 run 前的 `creating_run`，确定性代码点、零 LLM 成本）。**路由器相位 `understanding` 当日验收翻案撤销**——它与 "Thinking…" 同义零信息（用户判词：标签只在活动结构性不同于 thinking 时配拥有座位），行在路由期保持 "Thinking…"。前端据此换 thinking 行文案（未知相位落 `defaultValue` 回退，`{}` 帧不碰文案）；终态恰好一帧——`turn.completed`（完整 ChatResponse，**信封永远赢**，整体替换预览气泡）或 `turn.failed`（`{"detail"}`，流中失败，不落库）。15s 心跳注释帧。
...
- **打字机律（绝对规范，2026-09-08 用户拍板——「任何改动都不能违背」）**：agent 的一切言语必须以打字机节奏出现，整段瞬移永禁。三条结构牙：① **散文字段入 schema 必带读容忍**（`prose`/`default_path`/`options` 的 null 读为空默认值——一次 schema 拒收 = funnel 修复轮，而**修复轮永不流式**（两条 delta 流交错进一个气泡比信封换文更坏，N-26），于是整段瞬移）；② **extractor 的 null 跳过律**（上条）；③ **零 delta 路径的最后闸门 = `paceSettledProse`**：信封到达后散文照样走 typewriter 节拍释放（typewriter `drain()` 等排干），dock / QA 归档 / follow_up 一律等释放完成才落——sendChat 三分支（选项问 dock / 纯散文 / 文字问）与 streamAnswer 的 follow_up 全接。事故史：09-08 ask 散文瞬移——根因 = 新 `prose` 字段的 null 拒收（牙①缺）× extractor 的 null 锁死（牙②缺）× 零 delta 时信封散文即时推送（牙③缺），三陷阱同修 + 律法入 CLAUDE.md。
...
- **前端**：`lib/chat-stream.ts`（fetchEventSource POST，**禁自动重连**——重试会重复落用户消息；非 2xx 读 JSON detail 保持 422 toast 语义）。两个面同款节奏：发送 → thinking → 首个 delta 才建预览气泡（Streamdown `mode="streaming"` 渲染不完整 markdown）→ 信封**原地落定**（同 key finalize 转静态，**永不 remount**——删除重推会让已定散文重播入场动画 = 肉眼闪烁；answer 路径 2026-09-06 起同纪律：乐观 QA 块**原位**替换为归档行（同 index **同 key**——换 key = 卸载重建 = 入场动画重播的闪烁，09-08 产品试用逮到 randomUUID 换 key 漏网同修）、预览泡原地转静态，follow_up 散文由预览携带 echoCarried 不重推）+ 跑现有完成逻辑。**thinking 行只覆盖 send → 首个 delta**：预览泡一旦存在过本回合永不回归——`previewSeen` 门（2026-09-06 首回合闪烁根修；settle 窗口（旧行回退的 `await fetchPendingBrief`——行自完备后活路零拉取，见 §8.5 前）chatBusy 仍真且无 streaming 消息，`!streaming` 派生门会在已完成文本下方闪回 thinking 行）。静态历史消息同走 Streamdown（`mode="static"`）。
- **answer 端点同 /chat Accept 协商（2026-09-05 验收批翻案）**：SSE 模式推续聊散文的 `assistant.delta` 预览 + 终帧 `answer.completed`（完整 AnswerResponse）/`answer.failed`——旧拍板「保持 JSON」的前提是「点击低频 + start 无 LLM」，ADR-053 后选项点选已成主答路、且 slot 答的续聊跑整段 book path LLM（echo 散文在此生成），点选却看着整段散文瞬间砸下 = 反馈缺位。JSON 仍是默认（剧本 / pill-Start 零 LLM 路径不变）；起 run 前同 chat 路径 emit `creating_run` 相位帧。

### 8.7 RunTaskList 显示律（2026-09-04 立；2026-09-08 单行动态行翻案——Claude Code status line 解剖为用户参照）

- **动态行 = 全部 chrome（2026-09-08 用户两轮拍板——先合并、再纠正灰阶）**：打勾流 = 一行动态动作行 + 其下平铺步骤，没有第二层，**全程同一耳语灰阶（text-xs muted）**。**运行中**它是 CC 式 status line（● shimmer 碎碎念 = **现在正在做什么**——行文本永远动态 + 阶段自身耗时）**且自己就是 expand/collapse 开关**；**终态**是**同一行的收据形态**（✓ 计划标题 · 总耗时 · chevron——同字号同灰，ADR-057 §9 动作行一句话原位变化，不叠行）。被删的「☷ 标题」表头行（text-sm/font-medium/ListChecks）是错规格的第二层，**不是收据的合法形态**——标题归收据行、当前活动归动态行、总耗时归收据行，表头一无所有。节点行说 **WHAT**、动态行说 **NOW**，永不重话。dock 的 ThinkingRow 同构但**无读秒**（2026-09-06 用户拍板：相位标签 shimmer only——消息流里的思考行不竞赛计时；run 级时钟只在动态行/收据）。**RunStatusRow（输入组上方折叠打勾行）同批退役**：三形态机下消息流恒在（panel 滚动区常驻 / dock 历史一拍出），折叠行与碎碎念逐字重复——进度面 = 流内打勾流一处，唯一进度面律更纯；短单个操作同样由动态行承载（FLORA 式动作行与清单不再分家）。
- **行文本 builder-written**：`spec.summary` 服务端 preset（建步时任务名）/ 完成时量化重写，前端渲染 spec 字段；kind→copy 字典只剩碎碎念的渐进回退链（stage hint → kind 渐进形 → preset → raw kind）。
