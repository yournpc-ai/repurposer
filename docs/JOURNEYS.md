...
| 用户话术族 | 体验规格 | 系统支撑 | 状态 |
|---|---|---|---|
| 「字幕调小一点，改成黄色」（相对量） | 礼貌 echo → **读当前 spec** → op → 重渲染 → 收官报量化 | 读工具 + edit ops ✅；**op 参数面缺 size/color**（覆盖度池首条） | 🚧 覆盖度 |
