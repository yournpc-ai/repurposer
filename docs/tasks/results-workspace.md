...
> Status: 🚧 施工中（2026-08-06 立项；2026-08-10 重排：**闭环链全链本周（08-10~08-14）收口**——收口（overlay + 回填，已落）→ 中（结果网格 + 终态不跳转）→ 下（工作面：chat 列 / 舞台 / 检视器 / 双 modal 退役）→ 末（三档重跑接线 + 活图 spike + 血缘板 go/no-go 裁决），周五 08-14 联合验收）。dub 为全程载体卡。
> **2026-08-07 Flow 基座修订（ADR-036，用户拍板）**：只读图渲染扶正为共享基座 **FlowView**——原话"这一期先只暴露只读图，但该连线的连线、该有的节点是节点；只能通过 chat 修改，不变"。D6 overlay 规格修订（扇出主视觉 + FlowStrip 取代手风琴）；屏 3 "进度网格"升级为 **run 进度图**（编译期定死的死图 + SSE 状态动画，升正排产，不再是 spike）；闭环链（末）spike 收窄为**全史血缘板**；禁令 #10–#14 追加。
...
> 依据：STRATEGY §5（闭环优先 / 顾问姿态）；CHAT_ARCH §3.3 / §8；ADR-028 / ADR-032 / ADR-033 / **ADR-035**（DAG 用户化三切）；**RECIPES §7.1（配方=数据 schema——overlay 即其渲染器）**；PROGRESS 第二周闭环链；竞品证据 2026-08-06 评审（DECISION_MATRIX §F——Lovart 类单产物工作面 ×2、flow 类画布+流程智能体 ×2、ElevenCreative 配方 modal、图片编辑 modal、gallery 检视 overlay，共七组截图）。
> 用户裁决（2026-08-06，五条）：
...
> ④ **DAG 三切**：静态配方流程图 = 采纳；可操作画布 = 永久拒绝；运行期活图 = spike + 小白复述测试，本周随闭环链验收裁决。
> ⑤ **排期 = 闭环链**：本简报全部内容 + Recipe schema + dub 数据实例，PROGRESS 第二周（08-10~08-14）全链收口；分镜/生成产物线后移为"扩展配方类型"行。
...
| # | 决策 | 要点 |
|---|---|---|
| D1 | **终态不跳转** | run 终态：打勾流收官为 RunCard 聚合行 + 产物缩略条；中央区从进度网格翻成结果网格；无 toast、无自动关窗（闭环链·中 先落**网格版**——run 收官就地落结果页网格；闭环链·下 随工作面达完全体） |
...
| D10 | **双 modal 退役（闭环链·下）** | ClipDetailModal → 检视器；ChatModal → 对话列（全高左列，消息机器一寸不动）。**asset scope 会话退役**：工作面统一 project 会话 + 焦点产物上下文注入（§6 context 组装加一行"当前焦点 output"）；asset scope 的独有价值（auto target 注入 / revise 兜底）被焦点注入替代——判例随工作面落地执行，届时同步剧本 harness |
| D11 | **网格组件双挂载** | 结果页裸访 + 工作面中央区共用同一网格组件（props 驱动，网格重构写时即 workspace-ready） |
| D12 | **Before/After 对照** | 舞台级开关（按住看 Before），operations 快照 + `restore_version` 驱动，视觉类产物通用（竞品图片编辑 modal 的唯一采纳件） |

## 2. 六屏形态（终态，闭环链·下 完全体；屏 1 已随收口落地，闭环链·中 落屏 4 的页面版）
...
## 3. 分期与改动点（闭环链，PROGRESS 第二周全链收口）

### 闭环链·收口：dub 数据实例 + 检视 overlay + 串连入口
...
| 小白复述测试 + go/no-go（本周五 08-14 前；裁决问题收窄为"血缘板是否升正为默认中心"） | 裁决落 ADR-035/036 补记 |

## 4. 命名审计

| 名 | 义 | 备注 |
|---|---|---|
| 工作面 workspace | 项目页三区形态（对话列 + 中央区 + 检视器） | 入 NAMING §2（工作面落地时登记） |
...
1. **本周内**：dub 入口链全通——点卡 → 检视 overlay（发射区上传 + 发送）→ chat 接住首发，全程无断点（2026-08-08 修订：回填折返删除）。
2. **本周内**：dub 完全通路 v1——再加"对话定计划 → 生成 → 终态不跳转落结果网格（分数可见/动作抬出/chips）→ chat 精修"。
3. **本周五（08-14）**：工作面就绪——六屏全程一面；双 modal 退役无回退投诉；舞台指出/直编/对照可用；S1–S40 剧本无回归（asset scope 退役同步 harness）。
4. **本周五（08-14）**：活图 go/no-go——复述测试（小白看完能用自己的话讲出"它拿我的素材做了什么"）+ 使用数据，裁决落 ADR-035 补记；升正 = 地图成默认中心，不升 = 留检视模式零浪费。周五 🎯 Remix 精修就绪。
