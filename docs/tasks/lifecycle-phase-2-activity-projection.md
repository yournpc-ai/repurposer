...
- **Phase 2.5（Verification Contract Migration，2026-09-19 三批收口）**：Audit（只读取证，锚点 `3236fae`）→ Batch A `8f84227`（gate 5a/5b/5c 静态边界三门 + harness Activity-first strict co-fire 迁移 + S1 戳三拍 + gate 4 checkpoint 条款裁决）→ Batch B `e0a2c9d`/`527cd84`/`21c65c9`/`38883c1`/`047ee51`（Charge 等价钉 + blocker 谓词序钉 / 上传即出生全边界型 / `_sweep_activities` 路由缝 stub ×2 / 剧本 fixture 律消灭 S5·S7·S10·S20A worker 竞态 / gate 牙口正反探针固化）→ Batch C 收口（`docs/tasks/verification-contracts.md` Registry + 三原则 + Known Variance 四态登记 + final inventory diff）。**Charge Semantics 等价在册：`charge_semantics_ready ≡ has_pending_plan` 是当前合同的故意等价（ADR-063 终读），非测试缺失；「plan_ready ∧ charge 未就绪」不可达，未来引入 = ADR 翻案。** 纯 pytest 305 例全绿 + check_gates 全绿 + S1/S5/S7/S10/S11/S20A/S21 线上实证（残余单发红全部复跑即绿 = 在册 D 类方差）。**Phase 2.5 CLOSED；Phase 2 VERIFICATION CLOSED。** 步⑤（旧相位退役）仍 HOLD → Phase 3 Presentation Migration 联动裁决，残余旧相位依赖点（S10:2453 drafting 断言 / `_stream_reads` print fallback / `SCENARIO_ACTIVITY_LEGACY` 逃生舱）已在 Registry §6 登记。
