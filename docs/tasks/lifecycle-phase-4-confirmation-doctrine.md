...
| 修复批（三项悬案全落地） | **① R10 生效（P8 翻案）**：`confirmation_scope_ready` 删 `plan_prose` 合取——确认 scope = 卡载荷（tasks/brief/estimate），散文降叙事装饰，空散文 dock 可确认；T10b 改写为 R10 锁 + S19 恢复 Start 形（端到端验收座绿）。**② 触发落点复评（ADR-080 第二谓词补齐）**：落 dock 前复评 `is_pending_plan`，命中即整体静默（`trigger_turn_silenced_pending_plan_at_landing`）+ S22 常驻回归座（caption 答复 dock 计划于触发 loop 在途窗口）。**③ gatherer plan-scoped 收窄**：pinned exemplar 非源时排除出 Start 门控资产集（decompile 读字节不读处理态，对齐 U2 注释意图）——S16 全绿即证明。同批：S16 docstring 红座注转现在时；verification-contracts §4 三行出册（已 FIXED 不占额）+ §6 登记（T10b/S19/S16 迁移 + S22 新增）；ADR-087 R10 生效登记。验证 = compileall + 纯 pytest 352 绿 + check_gates OK + 剧本受影响座全绿（S19 Start 形 / S22 落点拍 / S16 复跑全绿 / S1·S11·S20 回归守卫；S1·S16 首跑红皆在册方差复跑即绿）+ 全量剧本复跑（见 commit 报告） | ✅ 2026-09-21 |
