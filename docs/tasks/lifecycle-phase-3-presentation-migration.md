...
- **未跑验证**：纯 pytest 全量 305 基线 / check_gates 全量 / 剧本 S5·S7·S10·S11·S20A（S5 在册方差，复跑即绿不算红）/ prompt_gate 三探针 round-robin（刀⑦）。
...
- **验收 PASS（2026-09-20，Claude 自跑——用户委托）**：纯 pytest **305/305** / check_gates **OK**（含 gate 5a/5b/5c）/ 剧本 S5·S7·S10·S20 绿——S5 turn1（工具调用参数倒进散文成截断 JSON）与 S20A（正则语序洞）首跑单发红、复跑即绿，均属在册方差/fragility 族（§4 S5 行同批扩记）；S11 红 = §6 在册延期债（`bilingual:'true'` 字符串形态两连同一形状，与本批零机制关联）/ **prompt_gate minimax 三探针 12/12 满分（A/B/C 合计 36/36，盖刀⑦+尾刀）**。另：C2 prod 守卫查询仍挂账（dev DB 实测 document×10+asset×5 分支活，`_read_face` 删除扳机未触发）。**Batch C CLOSED。Phase 3 CLOSED（A/B/C 三批全闭环）。**
