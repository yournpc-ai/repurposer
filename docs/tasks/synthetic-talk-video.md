...
> **Status（2026-08-05 修订）**：**声音路径后置**——R2 图片视频卡已按无声口径先行交付（照片轮播+字幕+音乐，走 `align_stills` 阅读节奏时间轴 + stills 短路径，RECIPES §4.2）。本简报的 `voice_gen`/`synth_visual`（TTS + 合成主片）设计整体留档，随 PROGRESS 第四~五周声纹/R5 线重启；重启时按当时的 registry/DAG 现状重核 §2 前置假设。
> **Base branch**: `main`（backend-module-restructure 已落地，路径已按新包结构核对）
> **Architecture reference**: `docs/AGENT_ARCHITECTURE.md`（四层工程地图）；ADR-029（双链并列）
> **Naming reference**: `docs/NAMING.md`（`voice_gen` / `synth_visual` 按 §5 注册零迁移）
> **Status**: 后置（声音路径随 PROGRESS 第四~五周声纹/R5 线重启，见上条 2026-08-05 修订）
> **Owner**: TBD
...
架构地基（ADR-029 已定，本任务不新决策）：生成 = RunPlan 新 node kind，与 clip 链共享 `workflow_steps` / worker / 计量 / 步骤清单；产物 `provenance=generated`；主轨生成物**不是 clip、不进 clip-spec**。
...
- Persona 画像：声纹挂载点（`clone_voice` 结果挂 persona）。
- `StepKind` 注册表守门（D6）：新 kind 注册零表迁移（NAMING §1 宪法第 5 条）。
...
2. `voice_gen`：文字稿 → persona 声纹 TTS → 音频 asset（`provenance=generated`）。
3. `synth_visual`：照片 + 音频 + 逐句字幕 + brand → Remotion 新 composition → **合成发言视频 asset**（`provenance=generated`，`source_ref` 指向文字稿 asset 与 persona）。
...
`StepKind` 注册两个新 kind + `STEP_RUNNERS` 两个 runner（架构迭代后 = 技能包节点类，ADR-039）：

| kind | runner 职责 | 产物 |
|---|---|---|
| `voice_gen` | 取文字稿 asset 文本 → persona 声纹（无则先 `clone_voice` 样本）→ `synthesize()` → 音频 asset 落库；TTS usage 经 metering 落 `workflow_steps.cost` | audio asset |
...
- Persona 已有声纹 → voice_gen 直接复用，UI 无新元素；
- Persona 无声纹 → composer 附件区接受音频样本（10s+），或在 pill 上标"需声音样本"引导；v1 不做录音 widget（上传文件即可）。
