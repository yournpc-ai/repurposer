...
> Status: **DONE**（2026-09-16 施工完毕：migration `f3a8c1d52e97` 两列 token / claim 铸 token / 全 失 Paths 置 NULL（10 处 render re-pend grep 复核）/ 四尾 + render 三写 token-guarded / fenced 零副作用纪律 / Suspend run 写 expected-from-state / 入口防御 ×2 / 纯测试 `tests/test_execution_fencing_pure.py` 9 项 / ADR-079 + ADR-017/030/050 修订 + North Star §8.1/§8.4/§13/§2 翻转。Implemented: 50c4ae8 / Verified: 2026-09-16 §10.1 自动化四条全绿 + **2026-09-17 §10.2–10.5 实弹演练全绿**（脚本 `apps/api/scratch/b2_drill_102.py` / `b2_drill_103.py` / `b2_drill_104_probe.py`——§10.2 节点 fencing：B 新 token/attempt+1 完成、`workflow_step_fenced` 落 A 日志、B 值不被覆写、capture 恰一条、钱包零二扣；§10.3 render 同型：`render_superseded` + 行保 B 的 files/token；§10.4 守卫实证 0 行；§10.5 回归归因入 `PROGRESS.md` §0.2）。**演练抓出并同批修掉真 bug**：fenced 五尾在 rollback 后读过期 ORM 属性组日志 → MissingGreenlet、`workflow_step_fenced` 永不落（fix 928d5a8）。排期唯一事实源 = `docs/PROGRESS.md` §0.1，本文件只是施工合同）
