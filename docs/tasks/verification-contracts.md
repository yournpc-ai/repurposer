...
| Scope 分类（continuation/expansion/unproven 三值 + 链逐字 retry 证明 + 零 op 语义可扩展锁） | `app/pipeline/scope_classifier.py` | ADR-087 §4 + D4/D2 + Frozen Rule 5/6/7/10 | `test_scope_classifier_pure.py`（27） | —（B2/B5 接线后补座） | — | 🟡 ｜ B1 additive——未接线，行为切换在 B2（edit_graph）/ B5（/generate）；gatherer 无纯测试座（lifecycle gatherer 先例） |
