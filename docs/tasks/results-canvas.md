...
> Status: 🚧 施工中（2026-08-11 立项，08-12 动工；周五 08-14 联合验收 + 画布转正复核）。
...
| D6 | **密度三档 + 过程脊** | 配方说明书 = 策展密度（≤5 节点，只画兑现承诺的步骤）；结果画布 = 骨架密度（素材 + 产物主角，中间步骤折为可展开"过程脊"组节点，带步骤计数）；run 期无图。**折叠是视图行为不是数据行为**——图数据永远全量（成本 / 重跑 / 血缘靠它），展示档由 NodeBase 自描述（与 `label()` 同哲学）。判定任一节点只问："隐藏它，用户会做错决定或失去信任吗？" |
...
**结果画布构图**：左 = 素材节点（asset 皮）；中 = 过程脊（默认折叠组节点，点击就地展开）；右 = 产物节点列（output 皮大卡）。边 = 真边（dependency / lineage 双语义，视觉可辨）。
