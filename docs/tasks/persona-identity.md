...
- 08-11：`voice` 缓存落地（dub 改读人设声纹，克隆一次跨项目复用）+ `STOCK_VOICES` 注册表 + 试听 + 声纹优先级链；**auto 生成收窄（08-10 用户拍板）**：bootstrap 提前退出扩到全默认解析链（有旧人设直接复用，不按项目繁殖）+ 意图门禁（纯 morph 需求——dub/翻译/去口头禅/配乐——不起 persona 节点、不生成人设；mode① 补 `needs_persona` 判定，persona=None 路径全库可跑通）+ 过渡清创（sidebar 撤 Brand template 项、`/brand-template` 重定向 `/personas`）
...
6. 人设不按项目繁殖：有旧人设的新项目直接复用默认链；纯配音/翻译/去口头禅类需求不产生新人设（auto 生成收窄，08-10 拍板）。
