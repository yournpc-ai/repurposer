...
- Speaker Profile 创建与管理（含风格画像、口吻设定）
- 项目级演讲素材上传（视频、音频、文字稿、幻灯片、图片）；输入场景含演讲 / 播客 / 网络研讨会 / 访谈
- 演讲者过往材料上传（书籍、文章、旧演讲、社媒内容）—— 用来读懂这个人
...
| ID | 需求 | 优先级 | 说明 |
|:---|:---|:---|:---|
| FR-006 | 创建项目 | P0 | 关联一个 Speaker，输入演讲标题/事件名称/日期 |
| FR-007 | 查看项目列表 | P0 | 按 Speaker/状态/日期筛选 |
| FR-008 | 删除项目 | P1 | 软删除 |
| FR-009 | 项目状态跟踪 | P0 | 上传中/处理中/待审校/已完成 |
| FR-010 | 选择 EU 数据驻留 | P0 | 创建项目时可选：默认（Global）/ EU 数据驻留（通过 Cast AI Kimchi） |
...
| ID | 需求 | 优先级 | 说明 |
|:---|:---|:---|:---|
| FR-018 | 自动生成 Speaker 风格画像 | P0 | 基于过往材料提取价值观、常用比喻、句式风格、专业术语偏好、情感倾向 |
| FR-019 | 人工编辑风格画像 | P0 | 用户可修改、补充、禁用某些表达 |
...
3. 上传演讲者过往材料
   ...
   ├── 社媒内容（文本或链接）
   └── 声音样本（可选，P1 做多语言配音）
4. 系统基于过往材料生成 Speaker 风格画像（Persona Agent）
5. 用户确认或编辑风格画像（可修改价值观、比喻、句式等）
...
6. 选择输出类型
   ...
   └── 是否生成 Carousel（P1）
7. 系统异步处理（显示进度：Persona → Analyze → Script → Review → Translate）
8. 用户进入审校页面
```
...
**不包含**：视频渲染、自动转写、幻灯片解析、声音克隆、复杂剪辑、社媒发布。
