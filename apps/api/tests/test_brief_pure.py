"""Tests for the Brief 顺形律 + 校验分层律 (ADR-064, 2026-09-11).
...
- 校验分层：brief 归一化后仍无法解析时丢字段不杀回合（关键载荷
...
class TestConstraintsNormalization:
    def test_canonical_shape_is_a_list_of_sourced_items(self):
        ...
        assert [c.value for c in brief.constraints] == [
            "keep 1:1",
            "no stock narrator",
        ]
        assert brief.constraints[0].source == BriefSlotSource.USER_STATED
    ...
    def test_legacy_object_wrapped_list(self):
        ...
        assert [c.value for c in brief.constraints] == ["a", "b"]
        assert all(c.source == BriefSlotSource.USER_STATED for c in brief.constraints)
    ...
    def test_legacy_object_wrapped_bare_string(self):
        ...
        assert [c.value for c in brief.constraints] == ["one long sentence"]

    def test_bare_string_items(self):
        brief = Brief.model_validate({"constraints": ["keep 1:1"]})
        assert [c.value for c in brief.constraints] == ["keep 1:1"]
    ...
    def test_off_enum_source_coerces_to_inferred(self):
        ...
        assert brief.constraints[0].source == BriefSlotSource.INFERRED

    def test_unrecognizable_shape_reads_as_no_opinion(self):
        """完全无法辨认的 constraints 按无意见处理——簿记不杀回合。"""
        brief = Brief.model_validate({"constraints": 42})
        assert brief.constraints == []

    def test_null_is_no_opinion(self):
        assert Brief.model_validate({"constraints": None}).constraints == []
...
class TestLayeredValidation:
    """校验分层律：brief 是咨询性簿记——garbled brief 丢字段，回合照活。"""
