...
Generates each catalog piece via MiniMax (``DEFAULTS_MODEL``) and uploads it to
object storage under ``music/{default_music_id(mood)}.mp3`` — the deterministic
uuid5 key that ``services/music.seed_default_music`` reconciles into ``Music``
rows at app startup. This script does NOT touch the DB; it only produces the
audio objects.

Idempotent: skips any mood whose object already exists, so re-running never
...
Requires ``MINIMAX_API_KEY`` and S3 credentials in the environment / .env.
"""
...
async def _seed_one(entry: dict[str, str], *, force: bool) -> bool:
    """Generate + persist one catalog piece. Returns True if an object was written."""
    mood = entry["mood"]
    music_id = default_music_id(mood)
    key = music_file_path(music_id)
    exists = await music_disk_path(music_id)
    if exists and not force:
        print(f"  [skip] {mood}: already exists at {key}")
        return False

    print(f"  [gen ] {mood}: '{entry['title']}' via {DEFAULTS_MODEL} …")
    generated = await generate_music(entry["prompt"], model=DEFAULTS_MODEL)
    file_path, size = await persist_music(music_id, generated)
    print(
        f"  [ok  ] {mood}: uploaded {size} bytes -> {file_path} "
...
async def _main(moods: list[str] | None, *, force: bool) -> int:
    ...
    print(f"\nDone. {written} object(s) written; run the API to seed Music rows.")
    return 0
...
def main() -> None:
    ...
    parser.add_argument(
        "--force", action="store_true", help="Regenerate even if the object exists."
    )
    parser.add_argument(
        "--mood",
        action="append",
