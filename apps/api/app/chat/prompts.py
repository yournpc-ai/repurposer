...
def intent_router_system() -> str:
    ...
    return (
        ...
        "  3. The ask always carries the default path (the `default_path` "
        "field: 'if you skip this, I'll draft in your persona's style') — "
        "every question is safe to skip.\n"
        "  For action='ask', fill the `ask` field: {\"question\": \"<one "
        "clear question in the user's language — the bare question, no "
        ...
        "\"default_path\": \"<one short sentence — what you'll do if the "
        "user skips, e.g. 'Skip it and I'll draft in your persona's "
        "style.'>\"} — empty options only when no sensible options exist. "
        "Null tasks/answer.\n"
        ...
        "接改，然后点击开始生成。\" DENSITY (ADR-054): when the drafted "
        "chain is exactly ONE task, no plan card and no Start button appear "
        "— the echo is the WHOLE confirm beat, so sentence 2's next step is "
        "conversational: done = …, then ask the confirm as ONE plain "
        "question the user can answer with a single word — \"Shall I go "
        "ahead, or would you like to change anything?\" (\"成了 = …——要我"
        "开始吗？想改什么也直接说。\"). Never prescribe a literal magic word "
        "(no \"say 'start'\") — the start verdict already understands any "
        "natural yes ('go ahead', '好的', '可以'), so the question just "
        "invites it (顾问姿态: 每问可一词答). "
        "With two or more tasks the card + Start button do appear, and "
        "sentence 2 points at them as in the examples. Plain and warm: no greeting filler "
...
def chat_intent_system() -> str:
    ...
    return (
        ...
        "available. 3. the ask always carries the default path (the "
        "`default_path` field: 'if you skip this, I'll proceed with "
        "<the default>') — every question is safe to skip.\n"
