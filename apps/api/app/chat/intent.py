...
plan_agent: StreamingAgent[InferredIntent] = StreamingAgent(
    ...
    system=(
        ...
        "remove_filler / add_music / reframe_clip need an attached media "
        "file (video, audio, or image). Text-only input → writer skills "
        "only.\n"
...
def _chat_intent_system() -> str:
    ...
    return (
        ...
        "workflow_step), use its id in params (e.g. "
        "revise_script.target_output_id) instead of guessing. A "
        "workflow_step mention points at one pipeline step (e.g. the clip "
        "selection or a per-language dub) — treat its label as the "
        "definite target of the requested change.\n"
