...
class IntentAgent:
    ...
    async def infer(
        ...
        system_prompt = (
            ...
            "- outputs: array of requested asset types. Valid values: clips, post, quotes, carousel, article.\n"
            "  Default to [\"clips\", \"post\", \"quotes\", \"article\"] when unclear.\n"
            "  If the user explicitly asks for only some types, return only those.\n"
            "  If the user says 'no clips', 'without clips', 'just a post', or excludes an output type, respect that.\n"
            "  Only include 'carousel' if the user explicitly asks for a carousel, slide deck, or swipeable post; otherwise leave it out.\n"
