"""Internal node crew (ADR-039 P1): the non-skill kernel runners.
...
(``app/skills/<pkg>/node.py``); the full ``STEP_RUNNERS`` table is assembled
by ``pipeline/registry.py``. Runner signature is uniform: ``(db, run, node,
project) -> list[UUID]`` — the ids of the output rows the node produced
(written to ``node.output_refs``).
"""
...
from app.pipeline.derivative_dispatch import _OUTPUT_TO_DERIVATIVE_TYPE
